...
public enum NativeRecoveryRoots {
    public static func provision(originals: Data, publicModel: String, request: String, model: String, prepared: String, output: String) throws {
        ...
        let config=NativeConfiguration(version:2,profile:AuthorityCodec.nativeProfile,provision:p,model:declaration,artifactPath:model); try config.validate()
    ...
    public static func initialize(root: String, role: String, configuration path: String, receipt: String, secretDescriptor: Int32) throws -> String {
        ...
        let config=try NativeConfiguration.load(path), executable=try LocalFiles.executable()
        ...
        do {
            ...
            let selection=try NativeSelection(version:1,profile:AuthorityCodec.nativeProfile,role:role.rawValue,configuration:config.digest,executable:executable)
    ...
    static func validateSelection(_ core: RoleBootstrapCore) throws -> NativeConfiguration {
        ...
        let config=try NativeConfiguration.load(core.root+"/agreement.json")
...
final class NativeRecoveryRootAccess {
    ...
    init(receipt path: String, expectedDigest: String) throws {
        ...
        configuration=try NativeRecoveryRoots.validateSelection(receipt.ready.core)
    }
    deinit { lease.close() }
    ...
    func withKeys(scope: AuthorityScope, owner: GenerationAuthorityOwner, credential: UnlockCredential,
        ...
        func verify() throws {
            ...
            _=try NativeRecoveryRoots.validateSelection(core); try check()
        }
        try verify()
