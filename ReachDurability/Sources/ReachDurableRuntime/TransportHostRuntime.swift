...
final class TransportHostRuntime {
    ...
    private let acquired: AcquiredTransportRoot, owner: DurableSessionLifecycle, profile: SelectedArtifactProfile, admission: TransportHostAdmission
    ...
    init(root: String) throws {
        let acquired = try AcquiredTransportRoot(root, role: .host)
        self.acquired = acquired; selection = acquired.selection
        try acquired.audit.journal(.host)
        let clock = SystemLifecycleClock()
        let owner = try DurableSessionLifecycle.reopen(at: root + "/bootstrap/host", identity: .init(incarnation: acquired.core.hostID, clock: clock, quota: acquired.core.policy.hostQuota), keys: LocalRuntimeOwner.hostKeys(acquired.keys), clock: clock)
        self.owner = owner
        do {
            admission = try TransportHostAdmission(root: root, selection: acquired.selection, core: acquired.core, key: acquired.keys.key(.hostCatalog))
    ...
    func connect(_ token: UUID, peerDigest: String) throws -> Data {
        ...
        let configuration = AdapterConfiguration(dialect: 2, model: selection.descriptor.model, optIn: true, ready: !owner.uncertain)
        let profile = self.profile, admission = self.admission
        adapter = try .init(configuration: configuration, owner: owner, authorization: selection.pins.hostAuthorization, expectedClientRoot: acquired.core.clientID, allowNew: admission.reservation == nil,
    ...
    func receive(_ raw: RawFrame, token: UUID) throws -> [Data] {
        ...
        case .accepted(let value):
            ...
            storedBinding = try owner.wireProviderBinding(ticket: lastTicket, authorization: selection.pins.hostAuthorization, generation: value.payload.reference.generationID)
    ...
    func publication(_ token: UUID, observe: Bool) throws -> Publication {
        ...
        if observe, let lastTicket, let attachment = lastStatus?.attachment {
            phase = try owner.wireNativeCheckpointPhase(ticket: lastTicket, authorization: selection.pins.hostAuthorization, attachment: attachment)
        }
    ...
    func disconnect(_ token: UUID) throws {
        ...
        retainedBatches = try owner.replayForClient(ticket: lastTicket, authorization: selection.pins.hostAuthorization, attachment: attachment, after: 0)
        try owner.detach(ticket: lastTicket, authorization: selection.pins.hostAuthorization, attachment: attachment)
        lastStatus = adapter.status
    }
    func cancelLocal() throws {
        ...
        lastStatus = try owner.wireCancelLocalReservation(namespace: reservation.reference.session.sessionID, generation: reservation.reference.generationID, requestDigest: reservation.requestDigest, authorization: selection.pins.hostAuthorization)
