...
final class RoleBootstrapFileSystem {
    let path: String
    ...
    init(path: String, fresh: Bool, role: BootstrapRole) throws {
        self.path = path; self.role = role; _ = try RootKeyCodec.parent(path)
    ...
    func scan() throws -> Set<String> {
        ...
        while true {
            ...
            guard ["bootstrap.lock","intent.json","ready.json","selection.tmp",role.rawValue].contains(name), names.count < 5 else { throw BootstrapError.invalid }
            let isJournal = name == role.rawValue
    ...
    func journal(_ role: BootstrapRole) throws -> String {
        guard role == self.role, try scan().contains(role.rawValue) else { throw BootstrapError.incomplete }; return path+"/"+role.rawValue
    }
