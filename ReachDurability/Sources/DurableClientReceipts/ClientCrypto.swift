...
enum ClientCrypto {
    ...
    struct AAD: Encodable { let version: Int; var authority: String? = nil; let mode: String?; let pair: String?; let root: String; let boot: String; let policy: String; let role: String; let record: String; let generation: String; let revision: UInt64 }
    ...
    static func seal(_ plain: Data, role: String, record: String, generation: String, revision: UInt64,
        ...
        let aad = try crEncode(AAD(version: e.authorityMode.storageVersion, authority:try e.authority?.digest, mode: e.authorityMode == .legacy ? nil : e.authorityMode.rawValue, pair: e.pairDigest, root: e.rootID, boot: e.boot, policy: e.policy, role: role, record: record, generation: generation, revision: revision))
        ...
        let body = Data(e.authorityMode.storageMagic.utf8)+crUInt(revision)+Data(record.utf8)+Data(generation.utf8)+combined
    ...
    static func inspect(_ input: Data, role: String, environment e: ClientEnvironment, rootKey: SymmetricKey) throws -> Frame {
        ...
        guard data.count >= overhead, data.count <= (role == "manifest" ? ClientLimits.manifest : ClientLimits.snapshot)+overhead,
              data.prefix(8) == Data(e.authorityMode.storageMagic.utf8),
