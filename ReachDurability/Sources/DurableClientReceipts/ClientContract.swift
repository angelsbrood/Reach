...
public struct ClientContext: Codable {
    ...
    func validate() throws {
        ...
        guard (version == 1 && authority == nil || version == 2 && authority.map(AuthorityCodec.isDigest) == true && revision == "s100-host-client-authority-v1" || version == 3 && authority.map(AuthorityCodec.isDigest) == true && revision == "s101-host-client-native-v1" && ["ordinary","guided","required","allowed"].contains(route)), projection == "s80-events-v1", crDigest(upstreamDigest), issued > 0,
