...
public enum ClientAuthorityMode: String {
    case legacy, independent, recoveryAuthority = "recovery-authority"
    var storageVersion: Int { switch self { case .legacy: 1; case .independent: 2; case .recoveryAuthority: 3 } }
    var storageMagic: String { switch self { case .legacy: "S83GCM01"; case .independent: "S95GCM02"; case .recoveryAuthority: "S100CR03" } }
}
...
public struct ClientLocalRetention: Codable, Equatable {
    ...
    public init(acceptance: AuthorityAcceptance) throws {
        ...
        version=2; self.acceptance=acceptance; anchor=original.anchor; deadline=original.deadline; cap=original.cap
    ...
    func validate() throws {
        if version == 2 {
            ...
            guard anchor == original.anchor, deadline == original.deadline, cap == original.cap, boot == s.client.boot,
...
extension ClientEnvironment {
    func check(_ retention: ClientLocalRetention?) throws {
        ...
        if authorityMode == .recoveryAuthority {
            guard let authority, let acceptance=retention.acceptance, retention.version == 2 else { throw ClientError.unavailable }
...
extension DurableClientReceipts {
    ...
    public func originalAuthority(_ context: ClientContext, ticket: Data, anchor: UInt64?) throws -> ClientAuthority {
        guard authorityMode != .recoveryAuthority, context.authority == nil else { throw ClientError.unavailable }
