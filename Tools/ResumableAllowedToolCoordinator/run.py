#!/usr/bin/env python3
"""Offline native check of the local Reach-owned allowed-tool coordinator candidate; retain logs, clean owned copies."""
import argparse
import hashlib
import json
import os
...
PATCH_PATH = 'Libraries/MLXLMCommon/ResumableToolGenerationCheckpointView.swift'
PRODUCT_PATHS = {'Package.swift',
 'README.md',
 'Sources/AllowedToolCoordinator/AllowedToolCheckpoint.swift',
 'Sources/AllowedToolCoordinator/AllowedToolContract.swift',
 'Sources/AllowedToolCoordinator/AllowedToolCoordinator.swift',
 'Sources/AllowedToolCoordinator/AllowedToolReplayInput.swift',
 'Sources/AllowedToolFixtures/Fixtures.swift',
 'Sources/AllowedToolWorker/main.swift',
 'Tests/AllowedToolCoordinatorTests/AllowedToolCheckpointTests.swift',
 'Tests/AllowedToolCoordinatorTests/AllowedToolCoordinatorTests.swift',
 'mlx-swift-lm.patch',
 'run.py'}
TEST_SOURCES = ['AllowedToolCoordinatorTests.swift', 'AllowedToolCheckpointTests.swift']
CASES = ['prose', 'schema', 'visible', 'precedence', 'probe-unicode', 'selected', 'guided-c0', 'pending', 'unicode', 'newline', 'before-eos', 'call-ready', 'interpass', 'second-c0', 'final-ready', 'emitted', 'cancel-call-ready', 'cancel-interpass', 'cancel-active', 'cancel-final-ready', 'incomplete', 'schema-incomplete', 'zero', 'length', 'unknown', 'llama']
...
def main():
    ...
    parser.add_argument('--companion-root', type=Path, help='Owned S79 development scratch included in observations')
    args = parser.parse_args()
    reach, candidate = args.reach.resolve(), Path(__file__).resolve().parent
    companion = args.companion_root
    if companion:
        if companion.is_symlink() or companion.parent != Path('/private/tmp') or not companion.name.startswith('reach-s79.'):
    ...
    root = Path(tempfile.mkdtemp(prefix='reach-allowed-tool-coordinator.', dir='/private/tmp')).resolve()
    ...
    outcome = {'result': 'FAIL', 'proof': 'local native Reach-owned allowed-tool coordinator',
        'reused': 'Accepted S72-S78 campaigns and S74 RC1 whole-output-history proof; no prior suite or old-binary rerun'}
    ...
    def resources(label):
        ...
        if allocated > 16*1024**3 or fixture_bytes > 256*1024**2 or free < 20*1024**3:
            raise RuntimeError('S79 resource ceiling/floor')
    ...
    try:
        ...
        if len(PRIOR_OUTPUTS) != 34 or {p: sha(lm/p) for p in PRIOR_OUTPUTS} != PRIOR_OUTPUTS:
            raise RuntimeError('34 prior composition output bindings')
        patch = candidate/'mlx-swift-lm.patch'
        paths = re.findall(r'^diff --git a/(\S+) b/(\S+)$', patch.read_text(), re.M)
        if paths != [(PATCH_PATH, PATCH_PATH)] or (lm/PATCH_PATH).exists():
            raise RuntimeError('sole additive dependency-view path')
        command('s79-patch-check', ['git', 'apply', '--check', str(patch)], lm, 30)
        command('s79-patch-apply', ['git', 'apply', str(patch)], lm, 30)
        if {p: sha(lm/p) for p in PRIOR_OUTPUTS} != PRIOR_OUTPUTS: raise RuntimeError('prior output changed')
        output = {PATCH_PATH: sha(lm/PATCH_PATH)}
        write_json(evidence/'composition.json', {'result': 'PASS', 'ordered_prerequisites': stack,
            'unchanged_34_prior_outputs': PRIOR_OUTPUTS, 's79_patch_sha256': sha(patch), 's79_outputs': output,
            'claim': 'sole additive read-only S76 view; actual retained native child restore; outer-owned discarded history; exact WireEvent and S78 public helpers'})
        write_json(evidence/'patched-source-sha256.json', {**PRIOR_OUTPUTS, **output})
        ...
        helper_paths = [p for p in PREREQUISITES['ResumableRequiredToolCoordinator'] if p.startswith('Sources/RequiredToolCoordinator/')]
        if len(helper_paths) != 3: raise RuntimeError('three unchanged S78 helper sources')
        for p in helper_paths:
            target = harness/p; target.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(reach/'Tools/ResumableRequiredToolCoordinator'/p, target)
            if sha(target) != PREREQUISITES['ResumableRequiredToolCoordinator'][p]:
                raise RuntimeError('S78 helper source copy mismatch')
        ...
        binary = bin_path/'AllowedToolWorker'
        ...
        test_log = command('focused-tests', ['xcrun', 'swift', 'test', *flags, '--skip-build', '--no-parallel', '--filter', 'AllowedToolCoordinatorTests'], harness, 300)
        selected = sorted(re.findall(r'func (test\w+)\(', ''.join((candidate/'Tests/AllowedToolCoordinatorTests'/n).read_text() for n in TEST_SOURCES)))
        passed = sorted(re.findall(r"Test Case '-\[AllowedToolCoordinatorTests\.\w+ (test\w+)\]' passed", test_log))
        started_methods = sorted(re.findall(r"Test Case '-\[AllowedToolCoordinatorTests\.\w+ (test\w+)\]' started", test_log))
        ...
        for name in CASES:
            ...
            for mode in ('produce', 'restore'):
                ...
                if row['result'] != 'PASS' or checkpoint.stat().st_size > 64*1024**2 or expected.stat().st_size > 96*1024**2:
                    raise RuntimeError('fresh worker result or fixture bound')
                if any(row[k] != 0 for k in ('restore_model_calls', 'restore_prefill_calls', 'restore_id_allocations')):
            ...
            for key in ('checkpoint_sha256', 'binding_sha256', 'suffix_sha256', 'suffix_batches', 'suffix_events', 'phase_at_cut', 'raw_at_cut', 'parser_state_at_cut', 'proposals_at_cut', 'guided_index_at_cut', 'guided_consumed_at_cut', 'pending_at_cut', 'incomplete_unicode_at_cut', 'newline_reset_at_cut', 'delivered_at_cut', 'delivered_final', 'route', 'ending'):
        ...
        if {p: sha(lm/p) for p in PRIOR_OUTPUTS} != PRIOR_OUTPUTS or sha(lm/PATCH_PATH) != output[PATCH_PATH]:
        ...
        outcome.update(result='PASS', new_xctest_methods=len(passed), fresh_process_pairs=len(matrix)//2,
            sequential_workers=len(matrix), worker_binary_sha256=sha(binary), maximum_checkpoint_bytes=max(x['checkpoint_bytes'] for x in matrix),
