#!/usr/bin/env python3
"""Offline native check of the local Reach-owned encrypted durable host store candidate; retain logs, clean owned copies."""
import argparse
import hashlib
import json
import os
...
PRODUCT_PATHS = set(["Package.swift","README.md","run.py","Sources/DurableHostStore/StoreContract.swift","Sources/DurableHostStore/StoreFileSystem.swift","Sources/DurableHostStore/StoreCrypto.swift","Sources/DurableHostStore/StoreManifest.swift","Sources/DurableHostStore/DurableHostStore.swift","Sources/DurableHostStore/DurableGeneration.swift","Sources/StoreFixtures/Fixtures.swift","Sources/StoreWorker/main.swift","Tests/DurableHostStoreTests/StoreTests.swift","Tests/DurableHostStoreTests/StoreRecoveryTests.swift","Tests/DurableHostStoreTests/StoreNativeTests.swift"])
TEST_SOURCES = ['StoreTests.swift', 'StoreRecoveryTests.swift', 'StoreNativeTests.swift']
...
class Worker:
    ...
    def __init__(self, binary, harness, env, logs, label, mode, route, path, incarnation, keys, cut='none', ordinal=-1, cursor=0):
        import base64
        ...
        self.joined = False
        try:
            self.process = subprocess.Popen([str(binary), mode, route, str(path), incarnation, str(read_fd), cut, str(ordinal), str(cursor)],
            ...
            os.close(read_fd)
        try:
            if os.write(write_fd, keys) != 64: raise RuntimeError('key pipe short write')
        except BaseException:
            self.join(kill=True)
            raise
        finally:
...
def lock_gate(binary, harness, env, fixtures, logs, resources):
    import uuid
    path, incarnation, keys = fixtures/'lock-store', str(uuid.uuid4()), new_keys()
    rows, owner = [], None
    def start(label, mode):
        return Worker(binary,harness,env,logs,label,mode,'ordinary',path,incarnation,keys)
    try:
        owner = start('lock-owner','lock-init'); row = owner.row()
        if row['kind'] != 'locked' or row['factories'] != 0: raise RuntimeError('initial ownership')
        rows.append(row)
        contender = start('lock-contender','contender')
        try:
            row = contender.row()
            if row['kind'] != 'busy' or row['factories'] != 0: raise RuntimeError('separate process lock refusal')
            rows.append(row); contender.join()
        finally: contender.join(kill=True)
        code = owner.join(kill=True)
        if code != -signal.SIGKILL: raise RuntimeError('owner real SIGKILL')
        rows.append({'kind':'killed-and-joined','pid':owner.process.pid,'exit_code':code})
        owner = start('exec-owner','exec'); row = owner.row()
        if row['kind'] != 'locked' or row['epoch'] != 2: raise RuntimeError('successor ownership')
        rows.append(row); owner.ack(0)
        # Observe the actual exec, not a guessed delay. /bin/sleep retains its PID.
        for _ in range(100):
            command = subprocess.check_output(['ps','-p',str(owner.process.pid),'-o','comm='],text=True).strip()
            if command.endswith('sleep'): break
            time.sleep(.02)
        else: raise RuntimeError('exec observation')
        contender = start('exec-contender','contender')
        try:
            row = contender.row()
            if row['kind'] != 'locked' or row['epoch'] != 3 or owner.process.poll() is not None:
                raise RuntimeError('close-on-exec lifetime')
            rows.append(dict(row, kind='acquired-while-exec-owner-alive')); contender.join()
        finally: contender.join(kill=True)
        owner.join()
        resources('lock-gate')
        return {'result':'PASS','observed':rows,'claim':'stable advisory lock, SIGKILL successor and actual exec lifetime; no lock descriptor duplication'}
    finally:
        if owner: owner.join(kill=True)
        if path.exists(): shutil.rmtree(path)

def drive(worker, client, partial=None):
    recovery, done = None, None
    try:
        while True:
            row = worker.row()
            if row['kind'] == 'cut':
                ...
                return {'death':row,'exit_code':code,'recovery':recovery}
            if row['kind'] == 'recovered': recovery = row
            elif row['kind'] == 'frame':
                if partial == 'first' or (partial == 'tail' and row['count'] == 3) or (partial == 'tail2' and row['count'] == 2):
                    client.accept(row, limit=1 if partial in ('first','tail') else None)
                    code = worker.join(kill=True)
                    if code != -signal.SIGKILL: raise RuntimeError('publication death')
                    return {'death':{'kind':'partial-publication','point':partial,'first':row['first'],'count':row['count'],'pid':worker.process.pid},
                            'exit_code':code,'recovery':recovery}
                client.accept(row); worker.ack(client.cursor)
            elif row['kind'] == 'done':
                worker.join()
                if row['high'] != client.cursor or row['prefills'] != 0 or row['mlx_peak_bytes'] > 128*1024**2:
                    raise RuntimeError('worker terminal/native bounds')
                return {'done':row,'recovery':recovery}
    finally:
        worker.join(kill=True)

def death_matrix(binary, harness, env, fixtures, logs, resources, evidence):
    import uuid, base64
    references, reference_traces, result = {}, {}, []
    for route in ('ordinary','guided','required','allowed'):
        path, incarnation, keys = fixtures/('reference-'+route), str(uuid.uuid4()), new_keys()
        client = Client()
        try:
            row = drive(Worker(binary,harness,env,logs,'reference-'+route,'initialize',route,path,incarnation,keys),client)
            references[route] = client.projection()
            reference_traces[route] = row['done']['traces']
            if row['done']['native_calls'] <= 0: raise RuntimeError('reference native execution')
        finally:
            if path.exists(): shutil.rmtree(path)
            del keys
        resources('reference-'+route)

    cases = [
        ('ordinary-c0', 'ordinary', 'beforeManifestReplace', 0, None, False),
        ('ordinary-blob', 'ordinary', 'afterCandidateBlob', 2, None, True),
        ('guided-replace', 'guided', 'afterManifestReplace', 2, None, True),
        ('guided-preack', 'guided', 'afterCommitBeforeAck', 2, None, True),
        ('required-postack', 'required', 'afterAckBeforePublication', 2, None, True),
        ('allowed-receipt-loss', 'allowed', 'none', -1, 'first', True),
        ('required-partial-tail', 'required', 'none', -1, 'tail', False),
        ('ordinary-terminal', 'ordinary', 'afterAckBeforePublication', -2, None, False),
    ]
    for label, route, cut, ordinal, partial, active in cases:
        # Ordinary's final ordinal is observed in a reference, never a hardcoded model trace.
        if ordinal == -2:
            # Its terminal batch has usage+finish; ask the publication path to die there.
            partial = 'tail2'; cut = 'none'
        path, incarnation, keys = fixtures/label, str(uuid.uuid4()), new_keys()
        client = Client()
        resources(label+'-before')
        try:
            worker = Worker(binary,harness,env,logs,label+'-producer','initialize',route,path,incarnation,keys,cut,ordinal)
            first = drive(worker,client,partial)
            if 'death' not in first: raise RuntimeError('declared cut not reached: '+label)
            # Receipt loss deliberately advertises an older cursor, requiring exact duplicates.
            cursor = 0 if partial == 'first' else client.cursor
            recovered = drive(Worker(binary,harness,env,logs,label+'-restore','recover',route,path,incarnation,keys,cursor=cursor),client)
            rec, done = recovered['recovery'], recovered['done']
            if active and (rec['terminal'] or rec['ordinal'] < 0 or done['native_calls'] <= 0):
                raise RuntimeError('four-route active native continuation: '+label)
            if label in ('required-partial-tail','ordinary-terminal') and (not rec['terminal'] or rec['restore_factories'] != 0 or done['native_calls'] != 0 or done['factories'] != 0):
                raise RuntimeError('terminal host replay acquired model')
            if partial == 'first' and client.duplicates == 0: raise RuntimeError('receipt-loss duplicate not exercised')
            if client.projection() != references[route]: raise RuntimeError('exact original event-array sequence mismatch')
            # Compare only after recovery has actually performed native continuation.
            for trace in done['traces']:
                matches = [t for t in reference_traces[route] if all(t[k] == trace[k] for k in ('kind','index','inputDigest','weights'))]
                if len(matches) != 1 or not trace['inputs']: raise RuntimeError('native continuation trace identity')
                reference = matches[0]
                count = len(trace['inputs'])
                if any(trace[k] != reference[k][-count:] for k in ('inputs','offsets','logits')):
                    raise RuntimeError('native continuation is not exact reference suffix')
            digest_state = hashlib.sha256()
            for raw,index in client.projection(): digest_state.update(raw); digest_state.update(index.to_bytes(4,'big'))
            digest = digest_state.hexdigest()
            for file in path.iterdir():
                if file.name != 'lock' and not file.read_bytes().startswith(b'S81GCM01'):
                    raise RuntimeError('store file is not ciphertext framing')
                data = file.read_bytes()
                if keys[:32] in data or keys[32:] in data: raise RuntimeError('key material in store')
            result.append({'case':label,'route':route,'result':'PASS','death':first['death'],'exit_code':first['exit_code'],
                'recovered_pid':done['pid'],'recovered_epoch':done['epoch'],'selected_ordinal':rec['ordinal'],
                'selected_high':rec['high'],'terminal_at_reopen':rec['terminal'],'restore_model_calls':rec['restore_calls'],
                'restore_factories':rec['restore_factories'],'continuation_model_calls':done['native_calls'],
                'client_events':client.cursor,'exact_duplicates':client.duplicates,'event_projection_sha256':digest,
                'mlx_peak_bytes':done['mlx_peak_bytes']})
            write_json(evidence/'matrix.json',result)
            print(label+': real death/recovery PASS',flush=True)
...
def main():
    ...
    parser.add_argument('--companion-root', type=Path, help='Owned S81 development scratch included in observations')
    args = parser.parse_args()
    reach, candidate = args.reach.resolve(), Path(__file__).resolve().parent
    companion = args.companion_root
    if companion:
        if companion.is_symlink() or companion.parent != Path('/private/tmp') or not companion.name.startswith('reach-s81.'):
    ...
    root = Path(tempfile.mkdtemp(prefix='reach-durable-host-store.', dir='/private/tmp')).resolve()
    ...
    outcome = {'result': 'FAIL', 'proof': 'S81 same-boot local encrypted store and native host recovery',
        'reused': 'Accepted S72-S80 native campaigns including S79/S74 RC1 and tiny Llama recurrence; no prior suite or old-binary rerun'}
    ...
    def resources(label):
        ...
        if allocated > 16*1024**3 or fixture_bytes > 1024**3 or free < 20*1024**3:
            raise RuntimeError('S81 resource ceiling/floor')
    ...
    try:
        ...
        if set(files) != PRODUCT_PATHS: raise RuntimeError('fourteen-product ceiling')
        ...
        write_json(evidence/'composition.json', {'result': 'PASS', 'ordered_prerequisites': stack,
            ...
            'claim': 'Unchanged S72-S79 composition; twelve exact S78/S79/S80 coordinator/provider sources; exact WireEvent; host store only'})
        ...
        helper_count = 0
        for name, target_name in [('ResumableRequiredToolCoordinator', 'RequiredToolCoordinator'), ('ResumableAllowedToolCoordinator', 'AllowedToolCoordinator'), ('ResumableMLXProvider', 'ResumableMLXProvider')]:
            ...
            for p in paths:
                ...
                helper_count += 1
        if helper_count != 12: raise RuntimeError('twelve unchanged coordinator/provider sources')
        ...
        gate_log = command('platform-tests', ['xcrun', 'swift', 'test', *flags, '--skip-build', '--no-parallel', '--filter', 'DurableHostStoreTests.StoreTests'], harness, 600)
        client_gate()
        lock_rows = lock_gate(binary, harness, env, fixtures, logs, resources)
        write_json(evidence/'lock-gate.json', lock_rows)
        test_log = gate_log + command('native-tests', ['xcrun', 'swift', 'test', *flags, '--skip-build', '--no-parallel', '--filter', 'DurableHostStoreTests.StoreRecoveryTests|DurableHostStoreTests.StoreNativeTests'], harness, 600)
        selected = sorted(re.findall(r'func (test\w+)\(', ''.join((candidate/'Tests/DurableHostStoreTests'/n).read_text() for n in TEST_SOURCES)))
        passed = sorted(re.findall(r"Test Case '-\[DurableHostStoreTests\.\w+ (test\w+)\]' passed", test_log))
        started_methods = sorted(re.findall(r"Test Case '-\[DurableHostStoreTests\.\w+ (test\w+)\]' started", test_log))
        ...
        matrix = death_matrix(binary, harness, env, fixtures, logs, resources, evidence)
        ...
        outcome.update(result='PASS', new_xctest_methods=len(passed), real_death_cases=len(matrix),
            worker_binary_sha256=sha(binary), maximum_mlx_peak_bytes=max(x['mlx_peak_bytes'] for x in matrix),
            platform_lock_gate='PASS', four_active_native_routes='PASS')
