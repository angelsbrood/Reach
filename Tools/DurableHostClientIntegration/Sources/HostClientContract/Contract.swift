...
public struct HandoffMessage: Codable {
    public var action: String
    public var path: String?, root: String?, key: Data?, ticketKey: Data?, ticket: Data?, context: Data?
    public var generation: String?, route: String?, expectedRoot: String?, witness: HandoffWitness?, frame: HandoffBatch?
    public var cursor: UInt64?, time: UInt64?, lifetime: UInt64?, create: Bool?, boundary: String?
    public var high: UInt64?, terminal: Bool?, phase: String?, disposition: String?, calls: Int?, factories: Int?, peak: Int?
    public var state: String?, result: Data?, effectID: Data?, effectName: Data?, arguments: Data?, allowed: Bool?
    public init(_ action: String) { self.action = action }
}
public enum HandoffPipe {
    private static func readExact(_ count: Int, from handle: FileHandle) throws -> Data {
        var result = Data()
        while result.count < count {
            guard let data = try handle.read(upToCount: min(65536, count-result.count)), !data.isEmpty else { throw HandoffError.closed }
            result.append(data)
        }
        return result
    }
    public static func read(from handle: FileHandle = .standardInput) throws -> HandoffMessage {
        // Class byte + big-endian length: both class-specific ceilings precede body allocation.
        let header = try readExact(5, from: handle)
        guard header[0] <= 1 else { throw HandoffError.invalid }
        let maximum = header[0] == 0 ? HandoffContract.control : HandoffContract.replayMessage
        let length = header.dropFirst().reduce(UInt32(0)) { ($0<<8)|UInt32($1) }
        guard length > 0, length <= maximum else { throw HandoffError.oversized }
        let data = try readExact(Int(length), from: handle)
        let message = try HandoffContract.decode(HandoffMessage.self, data, maximum: maximum)
        guard (message.frame == nil) == (header[0] == 0) else { throw HandoffError.invalid }
        if let frame = message.frame { _ = try frame.last() }
        return message
    }
    public static func write(_ message: HandoffMessage, to handle: FileHandle = .standardOutput) throws {
        let bytes = try HandoffContract.encode(message, maximum: message.frame == nil ? HandoffContract.control : HandoffContract.replayMessage)
        var length = UInt32(bytes.count).bigEndian
        try handle.write(contentsOf: Data([message.frame == nil ? 0 : 1])+withUnsafeBytes(of: &length) { Data($0) })
