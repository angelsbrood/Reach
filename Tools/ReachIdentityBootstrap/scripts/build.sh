#!/bin/sh
set -eu
[ "$#" -eq 2 ] || { echo 'usage: build.sh linux ABSOLUTE_OUTPUT_DIRECTORY' >&2; exit 2; }
target=$1
case "$target" in linux) ;; *) echo 'target must be linux (arm64)' >&2; exit 2 ;; esac
output=$2
...
chmod 0700 "$output"
cd "$root"
go build -trimpath -buildvcs=false -ldflags '-s -w -buildid=' -o "$output/reach-identity-bootstrap" ./cmd/reach-identity-bootstrap
chmod 0755 "$output/reach-identity-bootstrap"
...
shutil.copyfile(root.parents[1]/'LICENSE',out/'LICENSE');(out/'LICENSE').chmod(0o644)
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
raw=subprocess.check_output(['go','list','-deps','-json','./cmd/reach-identity-bootstrap'],text=True)
...
for name in ['go.mod','scripts/build.sh']:inputs[name]=sha(root/name)
inputs['../../LICENSE']=sha(root.parents[1]/'LICENSE')
binary=sha(out/'reach-identity-bootstrap')
...
(out/'BINARY.sha256').write_text(binary+'  reach-identity-bootstrap\n')
