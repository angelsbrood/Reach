...
sh -n "$script_dir/static-test.sh"
python3 - "$root" <<'PY'
from pathlib import Path
import os,subprocess,sys,tempfile
root=Path(sys.argv[1]).resolve();build=root/'scripts/build.sh'
with tempfile.TemporaryDirectory(prefix='bootstrap-guards-') as tmp:
 scratch=Path(tmp).resolve();alias=scratch/'checkout-alias';alias.symlink_to(root.parents[1],target_is_directory=True)
 fake=scratch/'bin';fake.mkdir();marker=scratch/'go-called'
 (fake/'go').write_text('#!/bin/sh\nprintf called > "'+str(marker)+'"\nprintf go1.99.0\\n\n');(fake/'go').chmod(0o755)
 env=dict(os.environ,PATH=str(fake)+os.pathsep+os.environ['PATH'])
 cases=[([], 'usage:'),(['windows',str(scratch/'out')],'target must'),(['darwin',str(scratch/'out')],'target must'),(['linux/amd64',str(scratch/'out')],'target must'),(['linux','relative'],'must be absolute'),(['linux',str(root/'out')],'outside the checkout'),(['linux',str(root.parents[1]/'out')],'outside the checkout'),(['linux',str(alias/'new/out')],'outside the checkout')]
