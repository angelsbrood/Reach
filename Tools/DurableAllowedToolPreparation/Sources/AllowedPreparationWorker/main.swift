...
struct Snapshot:Encodable {
    ...
    let family:String,descriptor:RequestPreparationContract.ModelDescriptor,traces:[AllowedNativeTrace]
...
do {
    ...
    guard ["reference","checkpoint-probe","checkpoint-guided","recover"].contains(mode),["llama","state"].contains(family),
    ...
    try Device.withDefaultDevice(Device(.cpu)) {
        let fixture=try AllowedNativeFixture(family:family),pair=try fixture.pair(root:root,fresh:mode != "recover");defer { pair.close() }
        if mode=="recover" { try pair.recover() }
        else { try pair.start(AllowedPreparationFixtures.request(args[5])) }
        ...
        for _ in 0..<2000 {
            ...
            if mode.hasPrefix("checkpoint"),let (pass,model)=fixture.models.last,model.calls>(pass.tokens.count+63)/64+2 {
                if mode=="checkpoint-probe" && pass.kind == .probe { break }
    ...
    print("S91 worker settled")
} catch { fputs("S91 worker refused: \(error)\n",stderr);exit(1) }
