#!/usr/bin/env python3
"""Owned S100 authority campaign: one cold-booted VM and one serial host witness."""
...
root = base / ('authority-' + a.label); root.mkdir(mode=0o700)

def raw(value): return json.dumps(value, sort_keys=True, separators=(',', ':'), ensure_ascii=False).encode()

class AuthorityVM(VM):
    def __init__(self, *args):
        self.witness = None; self.buffers = {}; self.events = {}; self.phaseResults = {}; self.witnesses = []; self.guestAllocated = 0; self.guestFixtures = 0
        super().__init__(*args)
    def baseline(self, label):
        super().baseline(label)
        assert all(sha(p) == h for p, h in BINDINGS.items()), 'current S100 rig binding'
    ...
    def phase(self, phase):
        denied = phase.startswith(('postboot-', 'cleanup'))
        profile = '(version 1)(allow default)(deny network*)'
        if denied: profile += '(deny file-read-data (subpath "' + self.guest + '/fixtures"))'
        command = [str(TART), 'exec', '-i', self.name, '/bin/launchctl', 'asuser', '503', '/usr/bin/sudo', '-H', '-u', 'threshold-auto',
                   '/usr/bin/sandbox-exec', '-p', profile, '/usr/bin/python3', self.guest + '/authority_guest.py', self.guest, phase,
                   'model-denied' if denied else 'ordinary']
...
vm = AuthorityVM(root, 'reach-s100-' + base.name.split('.', 1)[1] + '-authority-' + a.label)
vm.guest = '/Users/threshold-auto/' + vm.name
result = dict(result='RUNNING', scope='authority-only actual admitted empty records across receiver reboot', vm=vm.name, guest=vm.guest)
write(root / 'inputs.json', dict(executable=str(a.executable), executableSHA256=sha(a.executable), metallibSHA256=sha(a.metallib),
...
def collect(label):
    ...
    _, data, _ = vm.user_exec(label, ['/usr/bin/tar', '-cf', '-', '-C', vm.guest,
        'authority_guest.py', 'control', 'logs', 'reports', 'originals'], timeout=30)
...
try:
    ...
    with tarfile.open(bundle, 'w') as archive:
        files = [(a.executable, 'bin/reachd', 0o700), (a.metallib, 'bin/mlx.metallib', 0o600), (PRODUCT / 'authority_guest.py', 'authority_guest.py', 0o600)]
    ...
    for name in ['a', 'b', 'c']:
        vm.phase('original-' + name)
        collect('collect-original-' + name)
        assert vm.witness.pid == original_witness and vm.witness.poll() is None
        vm.stop(name + '-cold-stop'); vm.start(name + '-cold-boot')
        assert vm.witness.pid == original_witness and vm.hello['identity'] == original_identity
        vm.phase('postboot-' + name)
    result.update(result='PASS', actualColdReboot=True, originalWitnessPID=original_witness, originalWitnessIdentity=original_identity,
        hostExpiry='PASS', clientExpiry='PASS', blockingAge='PASS', observedLossAndReplacement='PASS', noPostbootNativeAuthority=True,
        originalAdmissionOncePerIndependentPair=True, serialPairs=3, liveOriginalRegistrations=6)
except BaseException as error:
