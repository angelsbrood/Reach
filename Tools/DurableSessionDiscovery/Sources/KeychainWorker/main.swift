...
do {
    ...
    _ = try RecoveryFixture.base(); let access = try RecoveryFixture.access(workers)
    ...
    var first = RecoveryMessage("ready"); first.metadataPreserved = true; try RecoveryPipe.write(first)
    while true {
        let request = try RecoveryPipe.read()
        do {
            var response = RecoveryMessage("ok"); let slot = request.slot ?? "primary"
            if request.action == "close" {
                guard containers.isEmpty else { throw BootstrapError.invalid }; try RecoveryPipe.write(.init("closed")); break
            }
            switch request.action {
            case "create":
                guard containers[slot] == nil, containers.count < 2, let password = request.password else { throw BootstrapError.invalid }
                let path = try RecoveryFixture.container(slot), container = try OwnedFileKeychain.create(at:path,password:password)
...
} catch { var response = RecoveryMessage.failure(error); response.action = "unavailable"; try? RecoveryPipe.write(response); exit(1) }
