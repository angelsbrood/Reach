...
enum RecoveryEncryption {
    struct AAD: Encodable { let revision=RecoveryLimits.revision; let binding:RecoveryBinding; let record:String, context:String; let issued:UInt64, expires:UInt64 }
    ...
    static func aad(_ live:LiveClientRecord, record:String, binding:RecoveryBinding) throws -> Data {
        try RecoveryCodec.encode(AAD(binding:binding,record:record,context:live.contextDigest,issued:live.issued,expires:live.expires))
    }
    static func seal(_ ticket:Data, live:LiveClientRecord, record:String, binding:RecoveryBinding, root:SymmetricKey) throws -> Data {
        ...
        let body=RecoveryEnvelopeBody(bootstrap:binding.bootstrap,core:binding.core,root:binding.clientRoot,record:record,context:live.contextDigest,cipher:combined)
    ...
    static func inspect(_ data:Data, binding:RecoveryBinding, root:SymmetricKey) throws -> RecoveryEnvelopeBody {
        ...
        guard b.version==1, crEqual(b.bootstrap,binding.bootstrap), crEqual(b.core,binding.core), crEqual(b.root,binding.clientRoot),
