#!/usr/bin/env python3
"""Offline native check of the local structural-guidance dependency candidate; retain logs, clean owned copies."""
import argparse
import hashlib
import json
import os
...
CHANGED_GRAMMAR = "Libraries/MLXGuidedGeneration/ResumableGrammarState.swift"
PATCH_PATHS = {CHANGED_GRAMMAR, "Tests/MLXGuidedGenerationTests/ResumableStructuralGuidanceTests.swift",
    "Tests/MLXGuidedGenerationTests/ResumableStructuralGuidanceCheckpointTests.swift"}
TEST_SOURCES = ["ResumableStructuralGuidanceTests.swift", "ResumableStructuralGuidanceCheckpointTests.swift"]
SELECTION = "ResumableStructuralGuidanceTests|ResumableStructuralGuidanceCheckpointTests"
CASES = ["c0", "pending", "name", "arguments", "unicode", "alternative", "complete", "incomplete", "cancelled", "llama", "ff-off"]
PRODUCT_PATHS = {"run.py", "README.md", "Package.swift", "mlx-swift-lm.patch", "Sources/StructuralGuidanceWorker/main.swift"}
...
def main():
    ...
    parser.add_argument("--companion-root", type=Path, help="Owned S77 development scratch to include in resource observations")
    args = parser.parse_args()
    reach = args.reach.resolve()
    candidate = Path(__file__).resolve().parent
    companion = args.companion_root
    if companion:
        if companion.is_symlink() or companion.parent != Path('/private/tmp') or not companion.name.startswith('reach-s77.'):
    ...
    root = Path(tempfile.mkdtemp(prefix='reach-mlx-structural-guidance.', dir='/private/tmp')).resolve()
    ...
    outcome = {'result':'FAIL','proof':'local native structural-tag guided checkpoint candidate',
        'reused':'Accepted S72-S76 campaigns and S74 RC1 output-history proof; focused old-kind compatibility only'}
    ...
    def resources(label):
        ...
        if allocated > 16*1024**3 or fixture_bytes > 64*1024**2 or free < 20*1024**3:
            raise RuntimeError('S77 resource ceiling/floor')
    ...
    try:
        ...
        if {p:sha(lm/p) for p in PRIOR_OUTPUTS} != PRIOR_OUTPUTS:
            raise RuntimeError('31 prior composition output bindings')
        patch = candidate/'mlx-swift-lm.patch'
        paths = re.findall(r'^diff --git a/(\S+) b/(\S+)$',patch.read_text(),re.M)
        if len(paths) != 3 or {a for a,b in paths} != PATCH_PATHS or any(a != b for a,b in paths):
            raise RuntimeError('three-path structural dependency delta')
        if any((lm/p).exists() for p in PATCH_PATHS if p != CHANGED_GRAMMAR): raise RuntimeError('S77 test outputs are not additive')
        baseline_grammar = (lm/CHANGED_GRAMMAR).read_bytes()
        command('s77-patch-check',['git','apply','--check',str(patch)],lm,30)
        command('s77-patch-apply',['git','apply',str(patch)],lm,30)
        preserved = {p:h for p,h in PRIOR_OUTPUTS.items() if p != CHANGED_GRAMMAR}
        if {p:sha(lm/p) for p in preserved} != preserved: raise RuntimeError('S77 changed another prerequisite output')
        patched_grammar = (lm/CHANGED_GRAMMAR).read_bytes()
        outputs = {p:sha(lm/p) for p in sorted(PATCH_PATHS)}
        write_json(evidence/'composition.json',{'result':'PASS','ordered_prerequisites':stack,'before_s77_31_outputs':PRIOR_OUTPUTS,'unchanged_30_prior_outputs':preserved,
            's77_patch_sha256':sha(patch),'s77_outputs':outputs,'claim':'full stack; actual native structural-tag compiler and unchanged S74 core/bridge'})
        write_json(evidence/'patched-source-sha256.json',{**PRIOR_OUTPUTS,**outputs})
        for name in ('TinyLlama','FocusedTests','Sources/StructuralGuidanceWorker'): (harness/name).mkdir(parents=True)
        for path in ('LLMModel.swift','Models/Llama.swift'):
            shutil.copy2(lm/'Libraries/MLXLLM'/path,harness/'TinyLlama'/Path(path).name)
        for name in TEST_SOURCES: shutil.copy2(lm/'Tests/MLXGuidedGenerationTests'/name,harness/'FocusedTests'/name)
        shutil.copy2(candidate/'Package.swift',harness/'Package.swift')
        shutil.copy2(candidate/'Sources/StructuralGuidanceWorker/main.swift',harness/'Sources/StructuralGuidanceWorker/main.swift')
        ...
        worker_text = (harness/'Sources/StructuralGuidanceWorker/main.swift').read_text()
        if test_text.split('import XCTest\n')[0] != worker_text.split('struct SGWorkerCase')[0]:
        ...
        binary = bin_path/'StructuralGuidanceWorker'
        ...
        passed = sorted(re.findall(r"Test Case '-\[MLXGuidedGenerationTests\.\w+ (test\w+)\]' passed",test_log))
        if len(selected) != 8 or selected != passed or 'Executed 8 tests, with 0 failures' not in test_log:
        ...
        print('8 focused native tests PASS',flush=True)
        matrix = []
        for name in CASES:
            ...
            for mode in ('produce','restore'):
                ...
                if row['result'] != 'PASS' or checkpoint.stat().st_size > 16*1024**2 or expected.stat().st_size > 32*1024**2:
            ...
            for key in ('checkpoint_sha256','specification_sha256','suffix_records_sha256','suffix_batches','pending_at_cut','ending'):
            ...
            if name == 'pending': shutil.copy2(checkpoint,fixtures/'new-structural.checkpoint')
            checkpoint.unlink(); expected.unlink()
        accepted_binary_sha = sha(binary)
        proven = private/'proven-worker'; proven.mkdir()
        new_binary = proven/'StructuralGuidanceWorker'
        shutil.copy2(binary,new_binary); shutil.copy2(reach/METALLIB,proven/'mlx.metallib')
        # Compile the exact accepted S74 grammar specification again. Only a private
        # worker setup branch unavailable to that old API is changed to permit linking;
        # old-kind runs and the decoded structural-refusal path never execute it.
        old_worker_source = worker_text.replace('case "structural-tag": specification = .init(structuralTag:item.source',
            'case "structural-tag": specification = .init(jsonSchema:item.source')
        if old_worker_source == worker_text: raise RuntimeError('old API compatibility overlay')
        (lm/CHANGED_GRAMMAR).write_bytes(baseline_grammar)
        if sha(lm/CHANGED_GRAMMAR) != PRIOR_OUTPUTS[CHANGED_GRAMMAR]: raise RuntimeError('old grammar byte binding')
        (harness/'Sources/StructuralGuidanceWorker/main.swift').write_text(old_worker_source)
        command('old-s74-compatibility-build',['xcrun','swift','build',*flags,'--product','StructuralGuidanceWorker'],harness)
        old_binary_sha = sha(binary)
        compatibility = []
        for name in ('old-json','old-literal'):
            checkpoint,expected = fixtures/(name+'.checkpoint'),fixtures/(name+'.expected')
            before = json.loads(command(name+'-old-produce',[str(binary),'produce',name,str(checkpoint),str(expected)],harness,60))
            after = json.loads(command(name+'-new-restore',[str(new_binary),'restore',name,str(checkpoint),str(expected)],harness,60))
            for key in ('checkpoint_sha256','specification_sha256','suffix_records_sha256','ending'):
                if before[key] != after[key]: raise RuntimeError('old/new compatibility binding '+key)
            if before['pid'] == after['pid']: raise RuntimeError('old/new fresh process')
            compatibility.append({'case':name,'old_producer':before,'new_restore':after})
            checkpoint.unlink(); expected.unlink()
        refusal = json.loads(command('old-s74-structural-refusal',[str(binary),'old-refusal',str(fixtures/'new-structural.checkpoint')],harness,60))
        if refusal['result'] != 'PASS': raise RuntimeError('old S74 structural refusal')
        write_json(evidence/'compatibility.json',{'result':'PASS','old_grammar_sha256':PRIOR_OUTPUTS[CHANGED_GRAMMAR],
            'old_binary_sha256':old_binary_sha,'new_binary_sha256':accepted_binary_sha,'old_to_new_pairs':compatibility,'old_structural_refusal':refusal})
        (fixtures/'new-structural.checkpoint').unlink()
        (lm/CHANGED_GRAMMAR).write_bytes(patched_grammar)
        (harness/'Sources/StructuralGuidanceWorker/main.swift').write_text(worker_text)
        if {p:sha(lm/p) for p in preserved} != preserved or {p:sha(lm/p) for p in PATCH_PATHS} != outputs:
            raise RuntimeError('final composition bindings')
        outcome.update(old_to_new_fresh_pairs=2,old_s74_structural_restore='refused',old_kind_encoded_identity='exact')
        authenticate_products()
        ...
        outcome.update(result='PASS',new_xctest_methods=len(passed),fresh_process_pairs=len(matrix)//2,sequential_workers=len(matrix),
            worker_binary_sha256=accepted_binary_sha,maximum_checkpoint_bytes=max(x['checkpoint_bytes'] for x in matrix),
