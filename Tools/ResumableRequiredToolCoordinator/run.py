#!/usr/bin/env python3
"""Offline native check of the local Reach-owned required-tool coordinator candidate; retain logs, clean owned copies."""
import argparse
import hashlib
import json
import os
...
PATCH_PATH = 'Libraries/MLXGuidedGeneration/ResumableGuidedCheckpointView.swift'
PRODUCT_PATHS = {'Package.swift',
 'README.md',
 'Sources/RequiredToolCoordinator/RequiredToolCheckpoint.swift',
 'Sources/RequiredToolCoordinator/RequiredToolContract.swift',
 'Sources/RequiredToolCoordinator/RequiredToolCoordinator.swift',
 'Sources/RequiredToolFixtures/Fixtures.swift',
 'Sources/RequiredToolWorker/main.swift',
 'Tests/RequiredToolCoordinatorTests/RequiredToolCheckpointTests.swift',
 'Tests/RequiredToolCoordinatorTests/RequiredToolCoordinatorTests.swift',
 'mlx-swift-lm.patch',
 'run.py'}
TEST_SOURCES = ['RequiredToolCoordinatorTests.swift', 'RequiredToolCheckpointTests.swift']
CASES = ['c0',
 'pending',
 'name',
 'arguments',
...
def main():
    ...
    parser.add_argument('--companion-root', type=Path, help='Owned S78 development scratch included in observations')
    args = parser.parse_args()
    reach, candidate = args.reach.resolve(), Path(__file__).resolve().parent
    companion = args.companion_root
    if companion:
        if companion.is_symlink() or companion.parent != Path('/private/tmp') or not companion.name.startswith('reach-s78.'):
    ...
    root = Path(tempfile.mkdtemp(prefix='reach-required-tool-coordinator.', dir='/private/tmp')).resolve()
    ...
    outcome = {'result': 'FAIL', 'proof': 'local native Reach-owned required-tool coordinator',
        'reused': 'Accepted S72-S77 campaigns and S74 RC1 whole-output-history proof; no prior suite or old-binary rerun'}
    ...
    def resources(label):
        ...
        if allocated > 16*1024**3 or fixture_bytes > 64*1024**2 or free < 20*1024**3:
            raise RuntimeError('S78 resource ceiling/floor')
    ...
    def command(label, cmd, cwd, timeout=900):
        ...
        with (logs/(label+'.log')).open('w') as log:
            ...
            try:
                ...
                raise
        commands.append({'label': label, 'command': cmd, 'exit_code': code, 'seconds': time.monotonic()-started_command})
        write_json(evidence/'commands.json', commands)
    ...
    try:
        ...
        if set(files) != PRODUCT_PATHS: raise RuntimeError('eleven-product ceiling')
        ...
        if len(PRIOR_OUTPUTS) != 33 or {p: sha(lm/p) for p in PRIOR_OUTPUTS} != PRIOR_OUTPUTS:
            raise RuntimeError('33 prior composition output bindings')
        ...
        command('s78-patch-check', ['git', 'apply', '--check', str(patch)], lm, 30)
        command('s78-patch-apply', ['git', 'apply', str(patch)], lm, 30)
        ...
        write_json(evidence/'composition.json', {'result': 'PASS', 'ordered_prerequisites': stack,
            'unchanged_33_prior_outputs': PRIOR_OUTPUTS, 's78_patch_sha256': sha(patch), 's78_outputs': output,
            'claim': 'sole additive read-only child view; Reach-owned coordinator uses actual native child and exact WireEvent'})
        ...
        binary = bin_path/'RequiredToolWorker'
        ...
        test_log = command('focused-tests', ['xcrun', 'swift', 'test', *flags, '--skip-build', '--no-parallel', '--filter', 'RequiredToolCoordinatorTests'], harness, 300)
        selected = sorted(re.findall(r'func (test\w+)\(', ''.join((candidate/'Tests/RequiredToolCoordinatorTests'/n).read_text() for n in TEST_SOURCES)))
        passed = sorted(re.findall(r"Test Case '-\[RequiredToolCoordinatorTests\.\w+ (test\w+)\]' passed", test_log))
        if not selected or selected != passed or f'Executed {len(selected)} tests, with 0 failures' not in test_log:
        ...
        for name in CASES:
            ...
            for mode in ('produce', 'restore'):
                ...
                if row['result'] != 'PASS' or checkpoint.stat().st_size > 32*1024**2 or expected.stat().st_size > 32*1024**2:
                    raise RuntimeError('fresh worker result or fixture bound')
                if row['restore_model_calls'] != 0 or row['restore_id_allocations'] != 0:
            ...
            for key in ('checkpoint_sha256', 'binding_sha256', 'suffix_sha256', 'suffix_batches', 'suffix_events', 'pending_at_cut', 'ending', 'entry_id', 'call_id'):
