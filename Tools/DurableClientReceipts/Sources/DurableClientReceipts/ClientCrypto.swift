...
enum ClientCrypto {
    ...
    struct AAD: Encodable { let version = 1; let root: String; let boot: String; let policy: String; let role: String; let record: String; let generation: String; let revision: UInt64 }
    ...
    static func seal(_ plain: Data, role: String, record: String, generation: String, revision: UInt64,
        ...
        let aad = try crEncode(AAD(root: e.rootID, boot: e.boot, policy: e.policy, role: role, record: record, generation: generation, revision: revision))
        ...
        let body = Data("S83GCM01".utf8)+crUInt(revision)+Data(record.utf8)+Data(generation.utf8)+combined
    ...
    static func inspect(_ input: Data, role: String, environment e: ClientEnvironment, rootKey: SymmetricKey) throws -> Frame {
        ...
        guard data.count >= overhead, data.count <= (role == "manifest" ? ClientLimits.manifest : ClientLimits.snapshot)+overhead,
              data.prefix(8) == Data("S83GCM01".utf8),
