...
public struct PFRow: Codable, Equatable {
    public let ordinal: UInt64
    public let events: Data
    public let phase: String
    public let terminal: Bool
}
public func pfRow(_ value: ProviderCandidate) throws -> PFRow {
    let cp = try value.checkpoint()
    return .init(ordinal: cp.ordinal, events: value.eventBytes, phase: cp.phase, terminal: cp.terminal != nil)
}
public func pfEvents(_ rows: [PFRow]) throws -> [WireEvent] {
    try rows.flatMap { try JSONDecoder().decode([WireEvent].self, from: $0.events) }
}
public final class PFHost {
    public let live: ResumableMLXProvider
    public let owner: String
    public var committed: ProviderCandidate
    public var rows: [PFRow]
    public init(_ setup: PFSetup) throws {
        owner = "owner-1"; live = try setup.prepare(owner: owner)
        guard let c0 = try live.pendingCandidate() else { throw CheckFailure("missing C0") }
        committed = c0; rows = [try pfRow(c0)]; try live.acceptCommit(c0.commit, owner: owner)
    }
    public init(restoring saved: ProviderCandidate, prefix: [PFRow], setup: PFSetup) throws {
        owner = "owner-2"; live = try setup.restore(saved, owner: owner); committed = saved; rows = prefix
        try check(try live.pendingCandidate() == nil, "restore publishes no pending event")
    }
    @discardableResult public func step(cancel: Bool = false) throws -> ProviderCandidate? {
        let value = try cancel ? live.cancel(owner: owner, current: committed.commit, credit: ResumableMLXProvider.reservationBytes)
            : live.advance(owner: owner, current: committed.commit, credit: ResumableMLXProvider.reservationBytes)
        if let value {
            rows.append(try pfRow(value)); try live.acceptCommit(value.commit, owner: owner); committed = value
        }
        return value
    }
    public func drain() throws { while try step() != nil {} }
    public func reach(_ boundary: String) throws {
        for _ in 0..<2000 {
            if try pfBoundary(committed, rows: rows, boundary) { return }
            guard try step() != nil else { throw CheckFailure("provider boundary absent: " + boundary) }
        }
        throw CheckFailure("provider finite fixture step bound")
    }
}
public func pfBoundary(_ saved: ProviderCandidate, rows: [PFRow], _ name: String) throws -> Bool {
    let cp = try saved.checkpoint()
    switch name {
    case "c0": return cp.ordinal == 0
    case "visible": return try pfEvents(rows).contains { if case .responseAppend = $0 { return true }; return false }
    case "pending":
        guard cp.binding.lane.route == .guided, cp.phase == "guided.active" else { return false }
        let d = try ResumableGuidedCheckpoint(data: cp.child).document().guided
        return d.grammar.accepts.count > d.consumed
    case "ready": return cp.phase == "required.ready"
    case "interpass": return cp.phase == "allowed.interpass"
    case "terminal": return cp.terminal != nil
    case "stop-prefix":
        guard cp.binding.lane.route == .ordinary else { return false }
        return try ResumableTextCheckpoint(data: cp.child).document().output.generationCount == 7
    default: throw CheckFailure("unknown provider boundary")
    }
}
public func pfAssert(_ host: PFHost, setup: PFSetup) throws {
    let events = try pfEvents(host.rows), cp = try host.committed.checkpoint()
    try check(host.live.isTerminal && cp.terminal != nil, "acknowledged semantic terminal")
    let tails = events.filter { if case .finished = $0 { return true }; return false }
    try check(tails.count == 1, "one ending")
    let text = events.compactMap { e -> String? in if case .responseAppend(_, let t, _, _) = e { return t }; return nil }.joined()
    ...
    if case .complete? = cp.terminal {
        var input = 5, output = 0
        switch setup.binding.lane.route {
        case .ordinary:
            let expected = setup.name == "ordinary-zero" ? "" : setup.native.text
            try check(Data(text.utf8) == Data(expected.utf8) && calls.isEmpty, "explicit ordinary projection")
            output = expected.utf8.count
        case .guided:
            input = 5; output = text.utf8.count
            if setup.name == "guided-structural" { try check(text == atEnvelope("alpha"), "structural response bytes") }
            else { try check(try JSONSerialization.jsonObject(with: Data(text.utf8)) as? [String: String] == ["value":"中"], "guided value") }
            try check(calls.isEmpty, "guided syntax is response text only")
        case .required:
            let expected: [WireEvent] = [.toolCallAppendArguments(entryID: "tool-entry-stable", id: "s80-call-stable", name: "alpha", content: "{\"n\":7}", tokenCount: 1)]
            try check(try encoder().encode(calls) == encoder().encode(expected), "exact required call/IDs")
            try check(text.isEmpty, "required syntax private"); output = try atEnvelope("alpha").utf8.count
        case .allowed:
            ...
            try check(try encoder().encode(calls) == encoder().encode(expected), "exact allowed ordered IDs/calls")
            try check(text == "前\nbetween\ntail", "allowed visible probe chunks")
        ...
        try check(usage == [.usage(inputTokens: input, outputTokens: output)], "explicit semantic usage")
        try check(events.suffix(2) == [.usage(inputTokens: input, outputTokens: output), .finished(.complete)], "whole ordered final tail")
    } else {
        try check(usage.isEmpty, "no invented successful usage")
        if setup.name == "ordinary-cancel" { try check(text == "helloST", "ordinary cancellation releases permitted stop prefix") }
    }
    try check(setup.prefills == 0 && Memory.peakMemory <= 128*1024*1024, "native model/tensor scope")
}
public func pfCompare(_ lhs: ProviderCandidate, _ rhs: ProviderCandidate) throws {
    var a = try lhs.checkpoint(), b = try rhs.checkpoint()
    let ac = a.child, bc = b.child
    // Future descriptor hashes may reflect tolerated native float differences.
    // Exact acknowledgement bytes are verified within each local execution.
    a.child = Data(); b.child = Data(); a.previousID = nil; b.previousID = nil
    try check(try encoder().encode(a) == encoder().encode(b) && lhs.eventBytes == rhs.eventBytes, "exact provider metadata/event frontier")
    func guided(_ x: Data, _ y: Data) throws {
        let xd = try ResumableGuidedCheckpoint(data: x).document(), yd = try ResumableGuidedCheckpoint(data: y).document()
        try equivalent(payload(xd.model), payload(yd.model))
        var xg = xd.guided, yg = yd.guided; xg.modelDigest = ""; yg.modelDigest = ""
        try check(try encoder().encode(xg) == encoder().encode(yg), "exact guidance frontier")
    }
    switch a.binding.lane.route {
    case .allowed: try atCompare(ac, bc)
    case .required:
        let x = try RequiredToolCheckpoint(data: ac).document(), y = try RequiredToolCheckpoint(data: bc).document()
        var xc = x.control, yc = y.control; xc.childDigest = ""; yc.childDigest = ""
        try check(try encoder().encode(xc) == encoder().encode(yc), "exact required control"); try guided(x.child, y.child)
    case .guided: try guided(ac, bc)
    case .ordinary:
        let x = try ResumableTextCheckpoint(data: ac).document(), y = try ResumableTextCheckpoint(data: bc).document()
        try equivalent(payload(x.raw), payload(y.raw))
        var xo = x.output, yo = y.output; xo.rawDigest = ""; yo.rawDigest = ""
        try check(try encoder().encode(xo) == encoder().encode(yo), "exact text frontier")
    }
}

public struct PFCase {
    public let fixture: String
    public let cut: String
    public var discard = false
}
public func pfCase(_ name: String) throws -> PFCase {
    let cases: [String: PFCase] = [
        "ordinary-c0": .init(fixture: "ordinary", cut: "c0"),
        "ordinary-visible": .init(fixture: "ordinary", cut: "visible"),
        "ordinary-discard": .init(fixture: "ordinary", cut: "visible", discard: true),
        "ordinary-terminal": .init(fixture: "ordinary", cut: "terminal"),
        "guided-pending": .init(fixture: "guided", cut: "pending"),
        "guided-terminal": .init(fixture: "guided", cut: "terminal"),
        "required-ready": .init(fixture: "required", cut: "ready"),
        "required-discard": .init(fixture: "required", cut: "ready", discard: true),
        "allowed-interpass": .init(fixture: "allowed", cut: "interpass"),
        "allowed-discard": .init(fixture: "allowed", cut: "interpass", discard: true),
        "allowed-terminal": .init(fixture: "allowed", cut: "terminal"),
        "llama": .init(fixture: "llama", cut: "pending")]
    guard let selected = cases[name] else { throw CheckFailure("unknown provider worker case") }; return selected
}
