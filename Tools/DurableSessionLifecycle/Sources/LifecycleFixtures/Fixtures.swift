...
public func lifecycleKeys() throws -> LifecycleKeys {
    func random() throws -> Data {
        var bytes = Data(count: 32)
        try check(bytes.withUnsafeMutableBytes { SecRandomCopyBytes(kSecRandomDefault, 32, $0.baseAddress!) } == errSecSuccess, "synthetic CSPRNG")
        return bytes
    }
    return try LifecycleKeys(catalog: random(), ticket: random())
}
public func cpuBinding(_ id: String = "operation", route: String = "ordinary") throws -> ProviderBinding {
    var binding = try Device.withDefaultDevice(Device(.cpu)) { try PFSetup(route).binding }
    binding.operationID = id; binding.requestID = "request:"+id; return binding
}
public func lifecyclePath() -> String {
    FileManager.default.temporaryDirectory.appendingPathComponent("lifecycle-"+UUID().uuidString.lowercased()).path
}
public func caller(_ name: String = "principal") -> LifecycleAuthorization {
    .init(caller: .init(principal: name, device: "device", app: "app"), allowed: true)
}
public func collect(_ owner: DurableSessionLifecycle, ticket: SessionTicket, auth: LifecycleAuthorization, attachment: LifecycleAttachment, cursor: inout UInt64) throws -> [StoreReplayFrame] {
    let frames = try owner.replay(ticket: ticket, authorization: auth, attachment: attachment, after: cursor)
    for frame in frames { cursor = frame.firstSequence+UInt64(frame.count)-1 }
    try owner.acknowledgeDelivery(ticket: ticket, authorization: auth, attachment: attachment, through: cursor)
    return frames
}
public func complete(_ owner: DurableSessionLifecycle, ticket: SessionTicket, auth: LifecycleAuthorization, attachment: LifecycleAttachment, setup: PFSetup, cursor: inout UInt64) throws -> [StoreReplayFrame] {
    var frames = try collect(owner, ticket: ticket, auth: auth, attachment: attachment, cursor: &cursor)
    for _ in 0..<600 {
        let state = try owner.step(ticket: ticket, authorization: auth, attachment: attachment) { setup.runtime }
        frames += try collect(owner, ticket: ticket, auth: auth, attachment: attachment, cursor: &cursor)
        if state.phase == .terminal { return frames }
    }
    throw CheckFailure("bounded lifecycle generation")
}
