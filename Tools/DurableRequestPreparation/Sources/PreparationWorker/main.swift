...
do {
    ...
    guard (mode=="recover" && args.count==4) || (mode != "recover" && args.count==5 && ["ordinary","required","required-eight"].contains(args[4])) else { throw PreparationError.unsupported }
    try Device.withDefaultDevice(Device(.cpu)) {
        let pair=try PreparationPair(root:root,fresh:mode != "recover");defer { pair.close() }
        if mode=="recover" {
            try pair.recover()
        ...
        else { try pair.start(PreparationFixtures.request(args[4]=="ordinary" ? "ordinary" : "required",n:args[4]=="required-eight" ? 8 : 7)) }
        let entryEncodes=pair.native.tokenizer.encodes
        for _ in 0..<600 {
            if pair.terminal { break }
            try pair.step()
            if mode=="checkpoint" && pair.native.model.calls>0 {
                let high=try pair.client.witness().high
                if args[4] != "ordinary" || high>0 { break }
            }
        }
        guard pair.native.model.calls>0,pair.native.model.prepares==0,Memory.peakMemory<=128<<20,
    ...
    print("S89 worker settled")
} catch { fputs("S89 worker refused: \(error)\n",stderr);exit(1) }
