#!/usr/bin/env python3
"""Owned S101 authority campaign: one cold-booted VM and one serial host witness."""
...
assert str(base).startswith('/private/tmp/reach-s101.') and base.stat().st_uid == os.getuid() and stat.S_IMODE(base.stat().st_mode) == 0o700
assert str(fixtures).startswith('/private/tmp/reach-s93.s101.') and fixtures.name == 'fixtures'
...
class NativeVM(VM):
    ...
    def baseline(self, label):
        ...
        assert all(sha(p) == h for p, h in BINDINGS.items()), 'current S101 rig binding'
        write(self.e / ('s101-rig-' + label + '.json'), BINDINGS)
    def sample(self):
    ...
    def join_witness(self, label, expected):
        ...
        receipt = dict(label=label, pid=p.pid, exitCode=code, joined=True, seconds=time.monotonic()-self.witnessStart, identity=self.hello['identity'])
    ...
    def phase(self, phase):
        ...
        if phase in ['terminal-primary','replacement-refusal'] or phase.startswith('cleanup'): profile += '(deny file-read-data (subpath "' + self.guest + '/fixtures/model"))'
...
vm = NativeVM(root, 'reach-s101-' + base.name.split('.', 1)[1] + '-native-' + a.label)
vm.guest = '/Users/threshold-auto/' + vm.name
result = dict(result='RUNNING', scope='ordinary native continuation across receiver cold reboot under S101 qualification assumptions' if a.campaign == 'full' else 'active ordinary native delay and witness refusal only; no reboot gate selected', vm=vm.name, guest=vm.guest)
write(root / 'inputs.json', dict(campaign=a.campaign, executable=str(a.executable), executableSHA256=sha(a.executable), metallibSHA256=sha(a.metallib),
    ...
    fixtureSHA256={str(p.relative_to(fixtures)): sha(p) for p in fixtures.rglob('*') if p.is_file()}))
...
try:
    ...
    if a.campaign == 'full':
        ...
        vm.phase('resume-primary'); vm.phase('terminal-primary'); vm.phase('retire-primary')
        vm.phase('reference'); vm.stop_witness('primary-reference-witness-complete')
        for name in ['host-expiry','client-expiry']:
            vm.start_witness('witness-' + name); vm.phase(name); vm.stop_witness(name + '-witness-complete')
        result.update(actualColdReboot=True, originalWitnessPID=original_witness, originalWitnessIdentity=original_identity,
            positiveNativeContinuation='PASS', exactReference='PASS', zeroArtifactTerminalReplay='PASS', hostExpiry='PASS', clientExpiry='PASS')
    vm.start_witness('witness-faults'); vm.phase('faults'); vm.phase('replacement-refusal'); vm.stop_witness('faults-witness-complete')
    result.update(result='PASS', campaign=a.campaign, blockingAge='PASS', observedLossAndReplacement='PASS',
