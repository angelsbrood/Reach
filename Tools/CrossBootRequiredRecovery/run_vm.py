#!/usr/bin/env python3
"""Owned S103 authority campaign: one cold-booted VM and one serial host witness."""
...
for name in ['scratch', 'fixtures', 'executable', 'metallib', 'retain']: parser.add_argument('--' + name, type=Path, required=True)
...
assert str(base).startswith('/private/tmp/reach-s103.') and base.stat().st_uid == os.getuid() and stat.S_IMODE(base.stat().st_mode) == 0o700
assert str(fixtures).startswith('/private/tmp/reach-s93.s103.') and fixtures.name == 'fixtures'
assert re.fullmatch('[a-z0-9-]+', a.label)
root = base / ('required-' + a.label); root.mkdir(mode=0o700)
...
class NativeVM(VM):
    ...
    def baseline(self, label):
        ...
        assert all(sha(p) == h for p, h in BINDINGS.items()), 'current S103 rig binding'
        write(self.e / ('s103-rig-' + label + '.json'), BINDINGS)
    def sample(self):
    ...
    def phase(self, phase):
        ...
        if phase in ['terminal-primary','duplicate-primary'] or phase.startswith('cleanup'): profile += '(deny file-read-data (subpath "' + self.guest + '/fixtures/model"))'
...
vm = NativeVM(root, 'reach-s103-' + base.name.split('.', 1)[1] + '-required-' + a.label)
vm.guest = '/Users/threshold-auto/' + vm.name
result = dict(result='RUNNING', scope='canonical one-required-tool native continuation across receiver cold reboot under S103 qualification assumptions' if a.campaign == 'full' else 'active required native delay only; no reboot gate selected', vm=vm.name, guest=vm.guest)
write(root / 'inputs.json', dict(campaign=a.campaign, executable=str(a.executable), executableSHA256=sha(a.executable), metallibSHA256=sha(a.metallib),
    sources={p.name: sha(p) for p in PRODUCT.glob('*.py')}, existingVMRecipeSHA256=sha(PRODUCT.parent / 'CrossBootRoleLifecycle/vm.py'),
    fixtureSHA256={str(p.relative_to(fixtures)): sha(p) for p in sorted((fixtures/'model').iterdir())+[fixtures/'requests/native-required.json']}))
payload = False; clean = False
fixtureFiles=sorted((fixtures/'model').iterdir())+[fixtures/'requests/native-required.json']
...
def collect(label):
    ...
    _, data, _ = vm.user_exec(label, ['/usr/bin/tar', '-cf', '-', '-C', vm.guest,
        'guest.py', 'control', 'logs', 'reports', 'originals'], timeout=30)
...
try:
    ...
    with tarfile.open(bundle, 'w') as archive:
        files = [(a.executable, 'bin/reachd', 0o700), (a.metallib, 'bin/mlx.metallib', 0o600), (PRODUCT / 'guest.py', 'guest.py', 0o600)]
    ...
    assert prepared['executableSHA256']==sha(a.executable) and prepared['metallibSHA256']==sha(a.metallib)
    assert prepared['modelSHA256']=={p.name:sha(p) for p in (fixtures/'model').iterdir()}
    if a.campaign == 'full':
        vm.start_witness('witness-original'); original_witness = vm.witness.pid; original_identity = vm.hello['identity']
        vm.phase('original-primary'); collect('collect-original-primary')
        vm.stop('primary-cold-stop'); vm.start('primary-cold-boot')
        assert vm.witness.pid == original_witness and vm.hello['identity'] == original_identity
        vm.phase('resume-primary'); vm.phase('ready-primary'); vm.phase('terminal-primary'); vm.phase('duplicate-primary'); vm.phase('retire-primary')
        vm.phase('reference')
        budget=vm.events['exact-common-preparation-frozen']['protocolBudget']
        assert vm.witnessCertificates==budget['combinedCertificates'] and vm.witnessRegistrations==4
        result['primaryReferenceCertificates']=vm.witnessCertificates
        vm.stop_witness('primary-reference-witness-complete')
        result.update(actualColdReboot=True, originalWitnessPID=original_witness, originalWitnessIdentity=original_identity,
            positiveRequiredContinuation='PASS', freshReadyZeroForwardDelivery='PASS', oneRegistrationDuplicateReplay='PASS', pendingForcedSuffix='PASS', exactReference='PASS', zeroArtifactTerminalReplay='PASS')
    vm.start_witness('witness-faults'); vm.phase('faults'); vm.stop_witness('faults-witness-complete')
    result.update(result='PASS', campaign=a.campaign, activeRequiredBlockingAge='PASS',
        originalAdmissionOncePerIndependentPair=True, serialPairs=3 if a.campaign == 'full' else 1, maximumLiveRoleKeychains=2)
