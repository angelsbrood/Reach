#!/usr/bin/env python3
"""Serial S103 actual native campaign. UID503 originals and keys stay in this guest."""
...
assert str(BASE).startswith('/Users/threshold-auto/reach-s103-') and os.getuid()==503
...
def controller(value):
    denied = PHASE in ['resume-primary','ready-primary','terminal-primary','duplicate-primary','retire-primary'] or PHASE.startswith('cleanup')
    excluded = [INPUTS,BASE/'fixtures/requests'] if denied else []
    if PHASE in ['terminal-primary','duplicate-primary'] or PHASE.startswith('cleanup'):excluded.append(BASE/'fixtures/model')
...
def fixture():
    ...
    request=json.loads((BASE/'fixtures/requests/native-required.json').read_bytes())
    assert request['options']['maximumResponseTokens']==48 and request['options']['toolCalling']=='required'
    assert len(request['tools'])==1 and not request.get('schema')
    ...
    for name in ['primary','reference']:
        command('prepare-'+name,['prepare-fixture','--model',BASE/'fixtures/model','--request',INPUTS/'request.json',
            '--operation','s103-original-common-operation','--output',INPUTS/(name+'-prepared.json'),'--public-model',INPUTS/(name+'-model.json')])
    ...
    # No witness, role Keychain, admission or durable generation exists yet.
    probe=command('required-feasibility',['probe-required-fixture','--model',BASE/'fixtures/model',
        '--prepared',INPUTS/'primary-prepared.json','--report',BASE/'reports/required-feasibility.json'])['frames'][0]
    assert probe['stage']=='required-fixture' and probe['beforeOriginals']
    cut=probe['cut'];ready=probe['ready'];m=probe['consumedTokens'];c=cut['guided']['consumedTokens']
    assert cut['phase']=='generating' and c>0 and cut['guided']['pendingTokens']>0
    assert ready['phase']=='ready' and ready['guided']['terminalReason']=='complete' and ready['guided']['interceptedEndings']==1
    assert 1<=probe['preparedTokens']<=512 and 1<=m<=48 and probe['initialPrepareCalls']==2
    ...
    e=json.loads((INPUTS/'primary-prepared.json').read_bytes());providerBytes=base64.b64decode(e['provider']);provider=json.loads(providerBytes)
    assert len(providerBytes)<=16384 and len(raw(probe))<=65536
    assert probe['providerBytes']==len(providerBytes) and probe['maximumStepFrameBytes']<=65536
    # Count the chosen protocol before originals. Reopen and terminalReplay each
    # consume a certificate, including both fresh terminal/duplicate workers.
    owners={'primaryCut':c+2,'resumedToReady':m-c+1,'freshReady':2,'terminalReplay':2,'duplicateReplay':2,'reference':m+3}
    primary=2+sum(owners[k] for k in ['primaryCut','resumedToReady','freshReady','terminalReplay','duplicateReplay'])
    reference=2+owners['reference'];combined=primary+reference
    budget=dict(consumedTokens=m,cutConsumed=c,ownerActions=owners,primaryCertificates=primary,referenceCertificates=reference,
        combinedCertificates=combined,nonceCeiling=128,nonceHeadroom=128-combined,registrationCount=4,registrationCeiling=16,
        worstCaseAt48Consumed=112,derivation='Primary M+11, reference M+5; each original admission and acceptance counts once. No authority reset.')
    assert max(owners.values())<=64 and combined==2*m+16 and combined<=128
    frames=[BASE/'reports/required-feasibility.json']+list((BASE/'reports/required-feasibility.json.steps').iterdir())
    frameSizes={str(p.relative_to(BASE/'reports')):len(p.read_bytes()) for p in frames};assert max(frameSizes.values())<=65536
    evidence=dict(preparedSHA256=sha(INPUTS/'primary-prepared.json'),provider=provider,requestSHA256=sha(INPUTS/'request.json'),
        publicModelSHA256=sha(INPUTS/'primary-model.json'),beforeEitherOriginal=True,protocolBudget=budget,actualFrameBytes=frameSizes,
        compiler=compiler,compilerSHA256=sha(compiler),sdk=sdk,cmathSHA256=sha(Path(sdk)/'usr/include/c++/v1/cmath'),
        launcherSHA256=sha(shim),nativeCompileDisabled=False,executableSHA256=sha(EXE),metallibSHA256=sha(BASE/'bin/mlx.metallib'),modelSHA256={p.name:sha(p) for p in (BASE/'fixtures/model').iterdir()})
...
def steps(report,label):
    ...
    for reference in report['requiredSteps']:
        ...
        result.append(item)
    return result
def run(pair,label,original=False,required='none',fault=None,boundary=None,expected=0,duplicate=False):
    ...
    item=command(label,args,pair=pair,checkpoint=checkpoint,boundary=boundary,expected=expected)
    if expected==0:
        ...
        assert data['actions']==len(item['certificates'])<=64
        ...
        if data.get('requiredProgress'):
            ...
            assert observed[-1]['progress']['guided']['commit']==data['selectedCommit']
        if required=='generating':
            v=data['requiredProgress'];g=v['guided']
            assert data['nativeCalls']>2 and data['client']['high']==data['hostHigh']==0 and not data['client']['terminal']
            assert v['phase']=='generating' and g.get('terminalReason') is None and g['acceptedTokens']>g['consumedTokens']>0
            assert g['pendingTokens']>0 and base64.b64decode(v['whole']) and all(x>0 for x in g['modelOffsets'])
        ...
        if required=='ready':
            v=data['requiredProgress'];assert v['phase']=='ready' and v['guided']['terminalReason']=='complete'
            assert v['guided']['interceptedEndings']==1 and data['hostHigh']==data['client']['high']==0
...
try:
    if PHASE in ['resume-primary','ready-primary','terminal-primary','duplicate-primary']:
        refused=[]
        probes=[INPUTS/'request.json',INPUTS/'primary-prepared.json',INPUTS/'primary-configuration.json',INPUTS/'primary-export.json']
        if PHASE in ['terminal-primary','duplicate-primary']:probes.append(BASE/'fixtures/model/profile.json')
        for probe in probes:
            try:
    ...
    if PHASE=='fixture':fixture()
        ...
        pair=original('primary',(600,900));data,item=run(pair,'primary-cut',original=True,required='generating')
        event('actual-required-generating-checkpoint',dict(report=data,workerPID=item['pid'],exitCode=item['exitCode'],joined=item['joined']))
    elif PHASE=='resume-primary':
        pair=load('primary');assert boot()!=pair['boot'] and hashes(pair)==pair['checkpointHashes']
        result,item=run(pair,'primary-resumed',required='ready')
        assert result['nativeCalls']>0 and result['beforeInbox']==result['inbox']==[]
        ...
        saved=pair['checkpoint']['requiredProgress'];progress=[step['progress'] for step in steps(result,'primary-resumed')]
        assert progress[0]==saved
        for step in range(1,saved['guided']['pendingTokens']+1):
            current=progress[step]['guided'];old=saved['guided']
        ...
        end=result['requiredProgress'];assert end['guided']['consumedTokens']<=48 and end['guided']['pendingTokens']==0
        assert json.loads(base64.b64decode(end['whole']))=={'name':'a','arguments':{'n':7}}
        pair['ready']=result;savepair(pair)
        event('actual-postboot-required-ready',dict(report=result,workerPID=item['pid'],exitCode=item['exitCode'],joined=item['joined'],savedForcedSuffixConsumedBeforeResampling=True))
    elif PHASE=='ready-primary':
        pair=load('primary');result,item=run(pair,'primary-ready-emission',required='emitted')
        assert result['requiredInitial']==pair['ready']['requiredProgress']
        assert result['requiredProgress']['phase']=='emitted' and result['requiredProgress']['call']==result['requiredInitial']['call']
        assert result['modelLoads']==1 and result['nativeCalls']==result['modelPrepares']==0
        assert result['client']['high']==0 and result['hostHigh']==3 and result['client']['registrations']==0
        pair['emitted']=result;savepair(pair);event('fresh-ready-zero-forward-whole-emission',result)
    elif PHASE=='terminal-primary':
        pair=load('primary');result,item=run(pair,'primary-terminal-replay')
        assert result['client']['terminal'] and result['client']['registrations']==1 and result['client']['high']==3
        assert result['nativeCalls']==result['modelLoads']==0 and not result['traces'] and not result['requiredSteps']
        assert result['replayedBeforeNative']==3 and result['beforeInbox']==[] and result['selectedCommit']==pair['emitted']['selectedCommit']
        assert not result.get('requiredProgress') and not result.get('requiredInitial')
        pair['terminal']=result;savepair(pair);event('terminal-replay-artifact-reads-denied',result)
    elif PHASE=='duplicate-primary':
        pair=load('primary');result,item=run(pair,'primary-duplicate-replay',duplicate=True)
        assert result['duplicateExact'] and result['client']==pair['terminal']['client'] and result['client']['registrations']==1
        ...
        event('fresh-client-exact-duplicate-one-registration',result)
        ...
        primary=load('primary');terminal=primary['terminal']
        assert result['provider']==terminal['provider'] and result['client']['terminal'] and result['client']['registrations']==1
        ...
        assert result['requiredProgress']==primary['emitted']['requiredProgress']
        def nativeTrace(reports,key):return [value for report in reports for trace in report['traces'] for value in trace[key]]
        split=[primary['checkpoint'],primary['ready'],primary['emitted']]
        for key in ['offsets','inputDigests']:assert nativeTrace(split,key)==nativeTrace([result],key)
        assert sum(x['nativeCalls'] for x in split)==result['nativeCalls']
        event('exact-independent-reference',dict(exactNativeEventBytes=True,exactNativeOffsetsAndInputs=True,primaryAdmission=terminal['admission'],referenceAdmission=result['admission'],report=result))
        retire(pair)
    elif PHASE=='faults':
        pair=original('faults',(180,240));active,_=run(pair,'faults-cut',original=True,required='generating')
        _,item=run(pair,'required-after-native-delay',fault='after-native',boundary='delay',expected=1)
        boundary=next(x for x in item['frames'] if x['stage']=='boundary')
        assert boundary['required']==active['requiredProgress']
        assert boundary['required']['guided']['pendingTokens']>0 and boundary['required']['phase']=='generating'
        assert item['seconds']>=11 and item['frames'][-1]['nativeCalls']==1
        event('required-after-native-eleven-second-refusal',dict(nativeCalls=1,noPublication=True,seconds=item['seconds'],
            activeRequired=boundary['required'],diskState='requires-authenticated-reconciliation'))
        retire(pair)
