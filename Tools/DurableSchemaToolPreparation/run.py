#!/usr/bin/env python3
"""S92 offline combined schema-and-tool preparation; no Keychain or old worker campaign."""
...
INVENTORY=Path("/private/tmp/reach-s91.wlqfxjvx/evidence/final-bindings.json")
INVENTORY_SHA="e484f01ade6f772c6878f01992126a2dd749aa79de015b796f5f25e31e9c29d4"
EXCEPTIONS={"Tools/DurableRequestPreparation/"+p for p in ["Sources/RequestPreparationContract/ModelDescriptor.swift","Sources/RequestPreparationContract/RequestPolicy.swift","Sources/DurableRequestPreparation/RequestPreparation.swift","README.md"]}|{"docs/wire.md"}
GIB=1<<30
...
def make_campaign(repo):
    class Campaign(campaign_class(repo)):
        def __init__(self):
            ...
            self.root=Path(tempfile.mkdtemp(prefix="reach-durable-schema-tool-preparation.",dir="/private/tmp"))
        ...
        def sample(self):
            ...
            if str(SOURCE).startswith("/private/tmp/reach-s92."):roots.append(SOURCE.parent)
        ...
        def prepare(self):
            ...
            inventory=json.loads(INVENTORY.read_text()); baseline=inventory["unchanged_S90_bindings"]|inventory["products"]
            assert len(baseline)==222
            ...
            for src in (self.repo/"Tools/DurableAllowedToolPreparation").rglob("*.swift"):
                relative=src.relative_to(self.repo/"Tools/DurableAllowedToolPreparation")
                if relative.parts[0] in ("Sources","Tests"):copy(src,self.package/relative)
            ...
            names=["SchemaToolPreparationWorker","ClientWireWorker"]
        ...
        def tests(self,prior=None):
            if prior is not None:
                prior=prior.resolve()
                assert prior.name=="evidence" and prior.parent.parent==Path("/private/tmp") and prior.parent.name.startswith("reach-durable-schema-tool-preparation.")
                assert json.loads((prior/"copied-libraries.json").read_text())==self.copied
                assert json.loads((prior/"composition.json").read_text())==json.loads((self.evidence/"composition.json").read_text())
                report=json.loads((prior/"tests.json").read_text())
                assert len(report["passed"])==49 and report["selected"]==report["started"]==report["passed"]
                report.update(execution="reused unchanged copied source and native composition",reused_from=str(prior/"tests.json"),reused_sha256=sha(prior/"tests.json"),raw_log_sha256=sha(prior.parent/"logs/tests.log"))
                save(self.evidence/"tests.json",report);return len(report["passed"])
            # S89-S91 tests share the changed contract and preparer.
            ...
            selection="DurableSchemaToolPreparationTests|DurableAllowedToolPreparationTests|DurableSchemaPreparationTests|DurableRequestPreparationTests|"+"|".join(old)
            ...
            selected=sorted(old+re.findall(r"func (test\w+)\(","\n".join(p.read_text() for folder in ("DurableSchemaToolPreparationTests","DurableAllowedToolPreparationTests","DurableSchemaPreparationTests","DurableRequestPreparationTests") for p in (self.package/"Tests"/folder).glob("*.swift"))))
        ...
        def workers(self):
            def run(mode,root,label,family,variant=None):
                ...
                args=[str(self.binary/"SchemaToolPreparationWorker"),mode,str(root),str(output),family]
            ...
            def response(s):return "".join(e["responseAppend"]["text"] for e in events(s) if "responseAppend" in e)
            ...
            def success(s,n=None):
                ...
                assert a["responseSchema"] and set(e[-2])=={"usage"} and e[-1]=={"finished":{"_0":{"complete":{}}}}
                ...
                if s["family"]=="llama":
                    assert not calls and json.loads(response(s))=={"n":n,"text":"The selected schema survives recovery."}
                    assert [t["kind"] for t in s["traces"]]==["probe","schema"]
                    assert all(t["preparedTokens"]==a["originalTokens"] for t in s["traces"])
                else:
                    assert [c["name"] for c in calls]==["alpha","beta"] and not response(s)
                    assert [json.loads(c["content"]) for c in calls]==[{"n":7},{"message":"Retain ordered values. "*8}]
                    assert [(t["kind"],t["index"]) for t in s["traces"]]==[("probe",0),("tool",0),("tool",1)]
                    assert json.loads(a["responseSchema"])["pattern"]=="^(?=a)a$" # Unused native-unsupported grammar.
                    probe=s["traces"][0];generated=[v for chunk in probe["inputs"] for v in chunk][len(probe["preparedTokens"]):]
                    assert generated[:len("PRIVATE PROBE PROSE. ")]==[b+1 for b in b"PRIVATE PROBE PROSE. "]
                    assert all(set(x) in ({"toolCallAppendArguments"},{"usage"},{"finished"}) for x in e)
            ...
            # Bound fallback feasibility first; these are not recovery oracles.
            for variant in ("eight","zero","short"):
                path=run("reference",self.fixtures/("llama-"+variant),"llama-"+variant,"llama",variant)
                ...
                if variant=="eight":success(s,8)
                    ...
                    assert not any("toolCallAppendArguments" in x for x in e)
                    assert [t["kind"] for t in s["traces"]]==["probe","schema"]
                    if variant=="zero":assert s["calls"]==0 and len(e)==1
                self.record("llama-"+variant,result="PASS",snapshot_sha256=sha(path),calls=s["calls"],events=e)
            for family,cut,variant in (("llama","schema","fallback"),("state","guided","calls")):
                reference=run("reference",self.fixtures/(family+"-reference"),family+"-reference",family,variant)
                label=family+"-"+cut;active=self.fixtures/(label+"-active")
                checkpoint=run("checkpoint-"+cut,active,label+"-checkpoint",family,variant)
                ...
                assert c["traces"][0]["kind"]==("schema" if cut=="schema" else "tool")
                if family=="llama":assert response(b) and response(a)==response(b)+response(c)
                else:assert not response(b) and not response(c)
        ...
        def finish(self,result):
            # Only these owned S92 fixture trees and the exact old-guard unit root.
...
def main():
    ...
    parser.add_argument("--reuse-tests",type=Path,help="Prior S92 evidence with identical copied source/native composition and 49 passing tests")
    args=parser.parse_args();c=make_campaign(args.repo.resolve());print(c.root,flush=True);result=dict(result="FAIL")
    try:
        c.prepare();tests=c.tests(args.reuse_tests);c.workers();c.verify();result=dict(result="PASS",tests=tests,binaries=c.binaries,worker_cases=len(c.matrix))
