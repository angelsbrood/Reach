#!/usr/bin/env python3
"""Offline native check of the local text-output dependency candidate; retain logs, clean owned copies."""
import argparse
import hashlib
import json
import os
...
PATCH_PATHS = {"Libraries/MLXLMCommon/" + p for p in [
    "Tokenizer.swift", "Evaluate.swift", "ResumableTextOutput.swift", "ResumableTextCheckpoint.swift",
]} | {"Tests/MLXLMTests/" + p for p in ["ResumableTextOutputTests.swift", "ResumableTextCheckpointTests.swift"]}
TEST_SOURCES = ["StreamingDetokenizerTests.swift", "ResumableTextOutputTests.swift", "ResumableTextCheckpointTests.swift"]
SELECTION = "ResumableTextOutputTests|ResumableTextCheckpointTests|StreamingDetokenizerTests"
CASES = ["unicode-c0", "unicode-buffer", "unicode-visible", "unicode-terminal",
         "stop-buffer", "stop-long-prefix", "stop-terminal", "llama-c0", "llama-visible"]
...
def main():
    ...
    root = Path(tempfile.mkdtemp(prefix="reach-mlx-text.", dir="/private/tmp")).resolve()
    ...
    outcome = {"result": "FAIL", "proof": "native tiny parser-disabled text-output dependency candidate only",
               "reused": "Accepted S72 native lane, 41-test/32-pair campaign and seven RC1 checks; not rerun as S73 evidence"}
    ...
    def resources(label):
        ...
        if allocated > 16 * 1024**3 or fixture_bytes > 64 * 1024**2 or free < 20 * 1024**3:
            raise RuntimeError("S73 resource ceiling/floor")
    ...
    try:
        ...
        if developer != "/Applications/Xcode-beta.app/Contents/Developer" or "swiftlang-6.4.0.33.1" not in swift:
            raise RuntimeError("selected S73 toolchain mismatch; no toolchain upgrade attempted")
        ...
        write_json(evidence / "inputs.json", {"reach_head": git(reach, "rev-parse", "HEAD"),
            "s72_sha256": S72_HASHES, "pins": PINS, "metallib_sha256": METALLIB_SHA, "developer": developer,
        ...
        worker_source = harness / "Sources/TextOutputWorker/main.swift"
        ...
        if not paths or len(paths) > 6 or len(set(a for a, _ in paths)) != len(paths) or any(a != b or a not in PATCH_PATHS for a, b in paths):
            raise RuntimeError("S73 dependency patch path ceiling")
        command("s73-patch-check", ["git", "apply", "--check", str(patch)], lm, 30)
        command("s73-patch-apply", ["git", "apply", str(patch)], lm, 30)
        if {a: sha(lm / a) for a in raw_hashes} != raw_hashes:
            raise RuntimeError("S73 changed accepted S72 source")
        write_json(evidence / "patch-stack.json", [
            {"slice": "S72", "patch_sha256": sha(raw_patch), "source_sha256": raw_hashes},
            {"slice": "S73", "patch_sha256": sha(patch), "source_sha256": {a: sha(lm / a) for a, _ in paths}},
        ])
        write_json(evidence / "patched-source-sha256.json", {**raw_hashes, **{a: sha(lm / a) for a, _ in paths}})
        ...
        if "Executed 16 tests, with 0 failures" not in test_log:
            raise RuntimeError("focused test count/selection mismatch")
        outcome["test_summary_lines"] = [line.strip() for line in test_log.splitlines() if "Executed " in line]
        print("16 focused native/legacy tests PASS", flush=True)
        matrix = []
        for name in CASES:
        ...
        outcome.update(result="PASS", selected_xctest_methods=16, fresh_process_pairs=len(matrix)//2,
        ...
        print("9 representative fresh-process output continuation pairs PASS", flush=True)
        if {p: sha(prerequisite / p) for p in S72_HASHES} != S72_HASHES:
