#!/usr/bin/env python3
"""Serial S102 actual native campaign. UID503 originals and keys stay in this guest."""
...
assert str(BASE).startswith('/Users/threshold-auto/reach-s102-') and os.getuid()==503
...
def fixture():
    ...
    request=json.loads((BASE/'fixtures/requests/native-guided.json').read_bytes())
    assert request['options']['maximumResponseTokens']==32 and request['context']['includeSchemaInPrompt']==False
    ...
    for name in ['primary','reference']:
        command('prepare-'+name,['prepare-fixture','--model',BASE/'fixtures/model','--request',INPUTS/'request.json',
            '--operation','s102-original-common-operation','--output',INPUTS/(name+'-prepared.json'),'--public-model',INPUTS/(name+'-model.json')])
    ...
    # Run the normal daemon empirically before the witness or any original role.
    probe=command('guided-feasibility',['probe-guided-fixture','--model',BASE/'fixtures/model',
        '--prepared',INPUTS/'primary-prepared.json','--report',BASE/'reports/guided-feasibility.json'])['frames'][0]
    assert probe['stage']=='guided-fixture' and probe['beforeOriginals'] and probe['cutFound']
    assert probe['progress'][-1]['terminalReason']=='complete'
...
def run(pair,label,original=False,checkpoint=False,fault=None,boundary=None,loss=False,expected=0):
    ...
    if checkpoint:args+=['--stop-with-pending-guided','--leave-host-ahead']
    ...
    if expected==0:
        ...
        if checkpoint:
            v=data['guidedProgress'][-1]
            assert data['nativeCalls']>1 and data['client']['high']>0 and not data['client']['terminal'] and data['hostHigh']>data['client']['high']
            assert v.get('terminalReason') is None and v['acceptedTokens']>v['consumedTokens']>0
            assert v['pendingTokens']>0 and base64.b64decode(v['cumulativeEmittedBytes'])
            assert all(x>0 for x in v['modelOffsets']) and v['commit']==data['selectedCommit']
            assert all(x['origin']=='forced' for x in v['accepts'][v['consumedTokens']:])
...
try:
    ...
    if PHASE=='fixture':fixture()
        ...
        event('actual-guided-pending-checkpoint',dict(report=data,workerPID=item['pid'],exitCode=item['exitCode'],joined=item['joined']))
        ...
        saved=pair['checkpoint']['guidedProgress'][-1];progress=result['guidedProgress']
        assert progress[0]==saved
        for step in range(1,saved['pendingTokens']+1):
            current=progress[step]
            assert current['sampledTokens']==saved['sampledTokens']
            assert current['forcedTokens']==saved['forcedTokens']+step
        ...
        assert result['client']['registrations']==0
        pair['terminal']=result;savepair(pair);event('actual-postboot-guided-continuation',result)
        ...
        assert result['nativeCalls']==result['modelLoads']==0 and not result['traces'] and not result['guidedProgress']
        assert result['selectedCommit']==pair['terminal']['selectedCommit']
        ...
        assert result['guidedProgress'][-1]==primary['guidedProgress'][-1]
        assert result['admission']!=primary['admission']
        ...
        pair=original('faults',(180,240));active,_=run(pair,'faults-cut',original=True,checkpoint=True)
        _,item=run(pair,'guided-after-native-delay',fault='after-native',boundary='delay',expected=1)
        boundary=next(x for x in item['frames'] if x['stage']=='boundary')
        assert boundary['guided']==active['guidedProgress'][-1]
        assert boundary['guided']['pendingTokens']>0 and boundary['guided'].get('terminalReason') is None
        assert item['seconds']>=11 and item['frames'][-1]['nativeCalls']==1
        event('guided-after-native-eleven-second-refusal',dict(nativeCalls=1,noPublication=True,seconds=item['seconds'],
            activeGuided=boundary['guided'],diskState='requires-authenticated-reconciliation'))
        retire(pair)
