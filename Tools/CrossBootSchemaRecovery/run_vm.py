#!/usr/bin/env python3
"""Owned S102 authority campaign: one cold-booted VM and one serial host witness."""
...
assert str(base).startswith('/private/tmp/reach-s102.') and base.stat().st_uid == os.getuid() and stat.S_IMODE(base.stat().st_mode) == 0o700
assert str(fixtures).startswith('/private/tmp/reach-s93.s102.') and fixtures.name == 'fixtures'
assert re.fullmatch('[a-z0-9-]+', a.label)
root = base / ('schema-' + a.label); root.mkdir(mode=0o700)
...
class NativeVM(VM):
    ...
    def baseline(self, label):
        ...
        assert all(sha(p) == h for p, h in BINDINGS.items()), 'current S102 rig binding'
        write(self.e / ('s102-rig-' + label + '.json'), BINDINGS)
    def sample(self):
...
vm = NativeVM(root, 'reach-s102-' + base.name.split('.', 1)[1] + '-schema-' + a.label)
vm.guest = '/Users/threshold-auto/' + vm.name
result = dict(result='RUNNING', scope='canonical schema-guided native continuation across receiver cold reboot under S102 qualification assumptions' if a.campaign == 'full' else 'active guided native delay only; no reboot gate selected', vm=vm.name, guest=vm.guest)
...
try:
    ...
    if a.campaign == 'full':
        ...
        result.update(actualColdReboot=True, originalWitnessPID=original_witness, originalWitnessIdentity=original_identity,
            positiveGuidedContinuation='PASS', pendingForcedSuffix='PASS', exactReference='PASS', zeroArtifactTerminalReplay='PASS')
    vm.start_witness('witness-faults'); vm.phase('faults'); vm.stop_witness('faults-witness-complete')
    result.update(result='PASS', campaign=a.campaign, activeGuidedBlockingAge='PASS',
