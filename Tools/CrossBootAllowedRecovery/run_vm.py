#!/usr/bin/env python3
"""Owned S104 authority campaign: one cold-booted VM and one serial host witness."""
...
assert str(base).startswith('/private/tmp/reach-s104.') and base.stat().st_uid == os.getuid() and stat.S_IMODE(base.stat().st_mode) == 0o700
assert str(fixtures).startswith('/private/tmp/reach-s93.s104.') and fixtures.name == 'fixtures'
assert re.fullmatch('[a-z0-9-]+', a.label)
root = base / ('allowed-' + a.label); root.mkdir(mode=0o700)
...
class NativeVM(VM):
    ...
    def baseline(self, label):
        ...
        assert all(sha(p) == h for p, h in BINDINGS.items()), 'current S104 rig binding'
        write(self.e / ('s104-rig-' + label + '.json'), BINDINGS)
    def sample(self):
...
vm = NativeVM(root, 'reach-s104-' + base.name.split('.', 1)[1] + '-allowed-' + a.label)
vm.guest = '/Users/threshold-auto/' + vm.name
result = dict(result='RUNNING', scope='normal Llama no-call and separately prescribed allowed-call native cold-reboot qualification' if a.campaign == 'full' else 'allowed next-pass native delay only; no reboot gate selected', vm=vm.name, guest=vm.guest)
write(root / 'inputs.json', dict(campaign=a.campaign, executable=str(a.executable), executableSHA256=sha(a.executable), fixtureExecutableSHA256=sha(a.fixture_executable), metallibSHA256=sha(a.metallib),
    ...
    fixtureSHA256={str(p.relative_to(fixtures)): sha(p) for p in sorted((fixtures/'model').iterdir())+sorted((fixtures/'state-model').iterdir())+[fixtures/'requests/native-allowed.json']}))
payload = False; clean = False
fixtureFiles=sorted((fixtures/'model').iterdir())+sorted((fixtures/'state-model').iterdir())+[fixtures/'requests/native-allowed.json']
...
try:
    ...
    with tarfile.open(bundle, 'w') as archive:
        files = [(a.executable, 'bin/reachd', 0o700), (a.fixture_executable, 'bin/reach-allowed-recovery-fixture', 0o700), (a.metallib, 'bin/mlx.metallib', 0o600), (PRODUCT / 'guest.py', 'guest.py', 0o600)]
    ...
    if a.campaign == 'full':
        for lane in ['normal','fixture']:
            ...
            if lane=='fixture':
                collect('collect-fixture-guided');vm.stop('fixture-guided-cold-stop');vm.start('fixture-guided-cold-boot')
                assert vm.witness.pid==original_witness and vm.hello['identity']==original_identity
                vm.phase('fixture-resume-guided')
        ...
        result.update(actualColdReboots=3, normalProseContinuation='PASS',prescribedCallContinuation='PASS',
    ...
    vm.start_witness('fixture-fault-witness');vm.phase('fixture-faults')
    assert vm.witnessCertificates==prepared['fixture']['protocolBudget']['certificates']['faults'] and vm.witnessRegistrations==2
    vm.stop_witness('fixture-fault-witness-complete')
    result.update(result='PASS',campaign=a.campaign,allowedNextPassBlockingAge='PASS',witnesses=witnesses,
