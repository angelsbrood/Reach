#!/usr/bin/env python3
"""Serial S104 actual native campaign. UID503 originals and keys stay in this guest."""
...
assert str(BASE).startswith('/Users/threshold-auto/reach-s104-') and os.getuid()==503
...
def feasibility():
    ...
    request=json.loads((BASE/'fixtures/requests/native-allowed.json').read_bytes())
    assert request['options']['maximumResponseTokens']==64 and request['options']['toolCalling']=='allowed'
    assert len(request['tools'])==1 and not request.get('schema');write(INPUTS/'request.json',request)
    bindings={}
    for lane in ['normal','fixture']:
        for who in ['primary','reference']:
            command(lane+'-prepare-'+who,['prepare-fixture','--model',model(lane),'--request',INPUTS/'request.json',
                '--operation','s104-original-common-operation','--output',INPUTS/(lane+'-'+who+'-prepared.json'),
        ...
        cut=next(i for i,x in enumerate(rows) if x['progress']['phase']=='probe' and x['progress']['probe']['rawTokens']>0 and x['progress']['proseDelivered']>0)
        ...
        def owner_actions(start,end,original=False):
            ...
            return actions
        owners={'probeCut':owner_actions(0,cut,True),'freshReady':owner_actions(ready,len(rows)-1),'terminal':2,'duplicate':2,'reference':owner_actions(0,len(rows)-1,True)}
        if lane=='fixture':
            guided=next(i for i,x in enumerate(rows) if x['progress']['phase']=='guided' and x['progress']['guided']['consumedTokens']>0 and x['progress']['guided']['pendingTokens']>0 and base64.b64decode(x['progress']['whole']))
            owners.update(resumeProbe=owner_actions(cut,guided),resumeGuided=owner_actions(guided,ready))
            assert probe['ready']['route']=='calls' and len(probe['ready']['proposals'])==len(probe['ready']['completedCalls'])==1
            route=next(i for i,x in enumerate(rows) if x['progress']['phase']=='routeReady')
            faultOwners={'routeCut':owner_actions(0,route,True),'delayedNextPass':2}
        else:
            owners['resumeProbe']=owner_actions(cut,ready)
            assert probe['ready']['route']=='prose' and not probe['ready']['proposals']
            faultOwners={}
        budgets={'primary':2+sum(v for k,v in owners.items() if k!='reference'),'reference':2+owners['reference']}
        if faultOwners:budgets['faults']=2+sum(faultOwners.values())
        assert max([*owners.values(),*faultOwners.values()])<=62 and max(budgets.values())<=120
        budget=dict(ownerActions=owners,faultOwnerActions=faultOwners,certificates=budgets,
            ownerCeiling=64,minimumOwnerHeadroom=64-max([*owners.values(),*faultOwners.values()]),nonceCeiling=128,
            minimumNonceHeadroom=128-max(budgets.values()),registrationsPerWitness=2,registrationCeiling=16,
            derivation='Measured committed step sequence; reopen, prepare, grouped advances, ready delivery, terminal replay and duplicate counted. Admission and acceptance add two. Each pair has its own original witness; reference has one uninterrupted generation owner.')
    ...
    command('normal-rejects-fixture',['prepare-fixture','--model',model('fixture'),'--request',INPUTS/'request.json',
        '--operation','s104-original-common-operation','--output',INPUTS/'forbidden-prepared.json','--public-model',INPUTS/'forbidden-model.json'],lane='normal',expected=1)
...
def run(pair,label,original=False,allowed='none',fault=None,boundary=None,expected=0,duplicate=False):
    ...
    if allowed!='none':
        ...
        # Normal probe prose is delivered before the cold boot. The fixture
        # probe deliberately retains a host-ahead prefix, replayed before work.
        if not (LANE=='normal' and allowed=='probe'):args+=['--leave-host-ahead']
    ...
    if allowed=='probe':
        ...
        assert data['nativeCalls']>data['modelPrepares'] and data['client']['registrations']==0
        if LANE=='normal':assert data['client']['high']>0 and data['client']['high']==data['hostHigh']
        else:assert data['hostHigh']>data['client']['high']
    ...
    if allowed=='ready':
        ...
        if LANE=='fixture':assert progress['guided']['interceptedEndings']==1 and len(progress['completedCalls'])==1
    return data,item
...
try:
    ...
    if ACTION=='feasibility':feasibility()
        ...
        result,item=resumed(pair,LANE+'-primary-probe-resumed','guided' if LANE=='fixture' else 'ready',pair['checkpoint'])
        if LANE=='fixture':
            prepared=[s for s in steps(result,LANE+'-primary-probe-resumed') if s['operation']=='next-pass']
            assert len(prepared)==1 and result['repairEncodes']>0
            assert prepared[0]['nativeCalls']==(len(prepared[0]['progress']['current']['tokens'])+255)//256
            assert prepared[0]['traces'][0]['offsets'][0]==0
        else:assert result['modelPrepares']==0;pair['ready']=result;savepair(pair)
        ...
        pair=load(LANE+'-primary');assert LANE=='fixture' and boot()!=pair['guidedBoot'] and hashes(pair)==pair['checkpointHashes']
        saved=pair['guidedCheckpoint'];result,item=resumed(pair,LANE+'-primary-guided-resumed','ready',saved)
        assert result['modelPrepares']==0 and result['repairEncodes']>0
        ...
        end=result['allowedProgress'];assert json.loads(base64.b64decode(end['whole']))=={'name':'a','arguments':{'n':7}}
        pair['ready']=result;savepair(pair)
        event('fixture-actual-guided-postboot-ready',dict(report=result,savedForcedSuffixConsumedBeforeResampling=True,workerPID=item['pid'],joined=item['joined']))
        ...
        split=[primary['checkpoint'],primary['guidedCheckpoint'],primary['ready'],primary['emitted']] if LANE=='fixture' else [primary['checkpoint'],primary['ready'],primary['emitted']]
        ...
        pair=original(LANE+'-faults',(180,240));active,_=run(pair,'fixture-fault-route-cut',original=True,allowed='route-ready')
        _,item=run(pair,'fixture-after-next-pass-delay',fault='after-next-pass-native',boundary='delay',expected=1)
        ...
        event('allowed-after-next-pass-eleven-second-refusal',dict(nativeCalls=2,noPublication=True,seconds=item['seconds'],
            activeAllowed=point['allowed'],lastAcceptedCommit=active['selectedCommit'],diskState='requires-authenticated-reconciliation'))
        retire(pair)
